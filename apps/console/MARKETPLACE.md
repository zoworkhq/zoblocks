...
Buying packs, components and themes from inside the console, and receiving them.
...
pnpm --filter @oxygenui-design/console db:seed         # two organisations
pnpm --filter @oxygenui-design/console db:seed:market  # five catalogue items
pnpm --filter @oxygenui-design/console dev             # http://localhost:6003
```
...
wants `CONSOLE_DEV_DB_PORT=59790 pnpm ... db:dev` and a matching
...
```bash
pnpm --filter @oxygenui-design/console stripe:sync            # prints the plan
pnpm --filter @oxygenui-design/console stripe:sync -- --apply # creates them
```
...
- **Raising invoices from the console.** The receiving half is built and tested;
