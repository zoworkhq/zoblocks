...
declare global {
  ...
  var __consoleMongo: { client: MongoClient; db: Db; cols: Collections } | undefined;
}
 ...
 * that site's Payload database. Honouring it would write the console's organisations and themes
 * into the CMS. Naming the database explicitly makes that impossible, whatever
 * URL is supplied.
 */
const DB_NAME = process.env.CONSOLE_DB_NAME || "oxygen_console";

function connect() {
  const uri = process.env.DATABASE_URL;
  if (!uri) {
    throw new Error(
      "DATABASE_URL is not set. Copy apps/console/.env.example to .env.local and point it at your MongoDB cluster.",
    );
  }
  ...
  client.on("topologyClosed", () => {
    if (globalThis.__consoleMongo?.client === client) globalThis.__consoleMongo = undefined;
  });
...
export function mongo() {
  globalThis.__consoleMongo ??= connect();
  return globalThis.__consoleMongo;
}
