 ...
 * themselves are thin — they read a token, call a function the console already
...
describe("the resolved theme", () => {
  it("carries the ramp, all three themes, and why the clinical tokens are fixed", async () => {
    ...
    // edited — in the same words the console uses.
...
describe("the draft endpoint", () => {
  it("derives the ramp from one anchor and hands back a console URL", async () => {
    const { asNorthwind } = await twoOrgs();
    const nw = await asNorthwind();
    const { token } = await withKey(nw);
    ...
    // Publishing stays a console action by a person with the role for it.
  ...
  it("records which key authored the change", async () => {
    ...
    // at the console — which is true about the actor and misleading about what
    // happened.
