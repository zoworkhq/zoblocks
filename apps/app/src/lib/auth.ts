...
export interface SessionMember {
   ...
   *
   * Read from the member document on every request rather than stored in the
   * session, so moving someone between organisations takes effect immediately
   * and there is one source of truth for which customer a request belongs to.
   */
  orgId: ObjectId;
  email: string;
  name: string;
...
function toSessionMember(doc: MemberDoc): SessionMember {
  return {
...
export async function resolvePasswordReset(
  ...
  const { passwordResets, members } = db();
  ...
  const user = await members.findOne({ _id: grant.userId });
...
export async function consumePasswordReset(token: string, newPassword: string): Promise<boolean> {
  ...
  const { members, passwordResets, sessions } = db();
  const hash = await hashPassword(newPassword);

  await members.updateOne({ _id: grant.userId }, { $set: { passwordHash: hash } });
...
export async function changePassword(
  ...
  const user = await members.findOne({ _id: userId });
...
export function scopesFor(user: SessionMember): string[] {
