...
describe("starting a purchase", () => {
  it("sends Stripe the catalogue's price, not the caller's", async () => {
    ...
    await createCheckout(auth, item.slug, "https://app.test", gateway);
  ...
  it("records the order before the customer leaves for Stripe", async () => {
    ...
    const { sessionId } = await createCheckout(auth, item.slug, "https://app.test", gateway);
  ...
  it("refuses an item that is not for sale", async () => {
    ...
    await expect(
      createCheckout(auth, item.slug, "https://app.test", fakeGateway()),
  ...
  it("refuses an unknown slug", async () => {
    ...
    await expect(createCheckout(auth, "nope", "https://app.test", fakeGateway())).rejects.toThrow(
      "No such item",
    );
  });
});
