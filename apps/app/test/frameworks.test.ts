 ...
 *
 * **The package graph.** The app must never resolve antd or MUI. That is
 * the architectural claim the whole bridge design rests on, and an admin
 * app that quietly depended on both would falsify it in its own manifest.
 */
...
describe("the app never resolves a UI framework", () => {
  /**
   ...
   * app does not declare it — this test names the rule so the failure is
   ...
   *   2. Inside the app, antd and MUI are reachable from exactly one file —
  ...
  it("reaches a UI framework from the export specimen and nowhere else", async () => {
    ...
    // One file, named. The app's own interface is Tailwind and its own kit;
