...
<AbsentValue />`,
  ...
  },
  {
    ...
    categories: ["Primitive", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Numeric with unit", "Comparator (<0.01)", "Non-numeric result", "Absent with a reason", "Absent with no reason", "Unit missing"],
    props: [
      { name: "quantity", type: "Quantity", description: "The measured quantity." },
      { name: "text", type: "string", description: "Non-numeric result — a titre, an organism, \u201cNonreactive\u201d." },
      { name: "absentReason", type: "CodeableConcept", description: "Passed to AbsentValue when there is no value. Absence renders as a statement, never a blank." },
      { name: "field", type: "string", description: "Field name, used for the accessible name of an absent value." },
      { name: "tone", type: "\"default\" | \"critical\" | \"high\" | \"low\" | \"normal\" | \"muted\"", default: "\"default\"", description: "Emphasis, not severity. Severity belongs to StatusBadge." },
      { name: "size", type: "\"sm\" | \"md\" | \"lg\" | \"hero\"", default: "\"md\"", description: "Scales for dense tables through patient-facing hero values." },
      { name: "hideUnit", type: "boolean", default: "false", description: "Hide the unit. Only when a column header already carries it." },
    ],
...
<ClinicalValue field="Magnesium" absentReason={observation.dataAbsentReason} />`,
    guidance: {
      use: ["Any place a measured quantity is displayed, in tables and in patient apps.", "With tone set from a resolved interpretation, never from a raw colour.", "With field set so an absent value announces which field it belongs to."],
      avoid: ["Re-rounding a value before passing it. Precision came from the source.", "Splitting the unit into a sibling element \u2014 that is the failure this exists to prevent.", "Using tone to convey severity on its own; pair it with a badge."],
    },
    accessibility: [
      { label: "One accessible name", detail: "The comparator, number, and expanded unit are announced as a single phrase, so a screen reader never reads a bare number." },
      { label: "Unit expansion", detail: "Common units are spoken in full \u2014 mg/dL and mmol/L differ by a factor that matters. Unknown units are announced as written rather than guessed at." },
      { label: "Tabular figures", detail: "Decimal points align down a column, which is what makes a dense results table scannable." },
    ],
    limitations: ["Unit conversion is not performed. Convert upstream and pass the value you want shown.", "The spoken-unit table covers common units only; an unlisted unit is read as written.", "Tone is presentational \\u2014 it does not derive severity from the value."],
  ...
  },
  {
    ...
    categories: ["Primitive", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Bounded range", "One-sided range", "No range available", "Value off scale", "Text-only range", "No value"],
    props: [
      { name: "value", type: "number", description: "The measured value." },
      { name: "range", type: "ObservationReferenceRange", description: "The range this observation carries. Never a global constant for the analyte." },
      { name: "interpretation", type: "Interpretation", default: "\"unknown\"", description: "Drives the marker colour. Pass the interpretation already resolved." },
      { name: "showBounds", type: "boolean", default: "true", description: "Show the numeric bounds beside the bar." },
      { name: "noRangeLabel", type: "string", default: "\"No reference range\"", description: "Text shown when no range exists. This is the common case, not an error." },
    ],
...
/>`,
    guidance: {
      use: ["Beside a value in a results table, where \\u201chow far out\\u201d is the real question.", "In trend and detail views where the band gives a value its context."],
      avoid: ["Supplying a range from a lookup table rather than the observation. Ranges are age-, sex-, and assay-conditional.", "Treating it as the severity signal. It is always paired with a badge and text.", "Expecting a bar for a text-only range \\u2014 it renders the text instead, deliberately."],
    },
    accessibility: [
      { label: "Decorative by design", detail: "The bar is hidden from assistive technology. The value, bounds, and interpretation are announced by the surrounding row; a nameless graphic would add noise, not access." },
      { label: "Off-scale survives greyscale", detail: "An off-scale marker changes shape as well as position, and is labelled in text, so the fact survives monochrome printing." },
      { label: "Bounds as text", detail: "Numeric bounds render as text beside the bar rather than as axis labels inside it." },
    ],
    limitations: ["Purely presentational \\u2014 it does not resolve which range applies to this patient.", "One-sided ranges infer the far end of the scale; it is marked, not hidden.", "Age- and sex-conditional range selection is the caller's job."],
  ...
  },
  {
    ...
    categories: ["Primitive", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Full instant", "Day precision", "Month precision", "Year precision", "Future timestamp", "Event vs recorded time", "No date"],
    props: [
      { name: "value", type: "string", description: "FHIR date, dateTime, or instant. Partial precision is honoured." },
      { name: "timeZone", type: "string", description: "IANA zone the instant is read in. Required \u2014 there is no safe default." },
      { name: "display", type: "\"absolute\" | \"relative\" | \"both\"", default: "\"absolute\"", description: "Relative never replaces the absolute form in the accessible name." },
      { name: "showZone", type: "boolean", default: "false", description: "Show the zone label. Recommended wherever zones can differ." },
      { name: "recorded", type: "string", description: "When the event was charted, if that differs from when it happened." },
      { name: "asOf", type: "Date", default: "new Date()", description: "Evaluation date. Pass a fixed date for deterministic tests." },
    ],
...
<ClinicalTime value={given} recorded={charted} timeZone={facilityZone} />`,
    guidance: {
      use: ["Every clinical timestamp, with the facility or patient zone passed explicitly.", "With recorded set wherever charting time can differ from event time.", "With showZone on any surface where participants can be in different zones."],
      avoid: ["Relying on the browser zone. There is no default here on purpose.", "display=\\u201crelative\\u201d in handovers or medication records, where the absolute time is the point.", "Passing a pre-formatted string \\u2014 pass the raw FHIR value so precision survives."],
    },
    accessibility: [
      { label: "Absolute always announced", detail: "The accessible name carries the absolute date, time, and zone even when the visible text is relative." },
      { label: "Real time element", detail: "Renders a <time> element with a machine-readable dateTime attribute." },
      { label: "Future flagged in text", detail: "A future timestamp is labelled in words, not by colour alone." },
    ],
    limitations: ["Relative phrasing is coarse by design and stops at 30 days.", "Daylight-saving ambiguity is not resolved \\u2014 the zone is applied as given.", "Relative display is ignored for year and month precision, which have no instant."],
  ...
  },
  {
    ...
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Complete identity", "Name not recorded", "Restricted record", "Deceased", "Photo without consent", "Name-alike collision", "Masked identifiers"],
    props: [
      { name: "patient", type: "Patient", description: "FHIR R4 Patient resource." },
      { name: "identifierSystem", type: "string", description: "Identifier system to display, e.g. your MRN system." },
      { name: "maskIdentifiers", type: "boolean", default: "false", description: "Mask all but the last four characters. For shared and public screens." },
      { name: "photoConsent", type: "boolean", default: "false", description: "Assert that the patient consented to their photo being displayed. Required before any photo renders." },
      { name: "size", type: "\"sm\" | \"md\" | \"lg\"", default: "\"md\"", description: "Avatar and name scale." },
      { name: "avatarOnly", type: "boolean", default: "false", description: "Avatar only. The full name still reaches assistive technology." },
      { name: "nameAlike", type: "boolean", default: "false", description: "Another patient in this view has a confusingly similar name. Renders an explicit warning." },
    ],
...
<IdentityToken patient={patient} nameAlike />`,
    guidance: {
      use: ["Worklists, care-team panels, search results, and anywhere a person is named.", "With nameAlike set by the list when two entries are confusable.", "With maskIdentifiers on shared workstations and public-facing screens."],
      avoid: ["Passing photoConsent as a constant true. It is an assertion about the patient, not a display preference.", "Using the avatar colour to mean anything. It is decorative and stable, nothing more.", "Omitting the identifier in a list where two patients could share a name."],
    },
    accessibility: [
      { label: "Full name never truncated", detail: "The visible name may truncate; the accessible name carries the whole identity, age, identifier, and flags as one phrase." },
      { label: "Flags as text", detail: "Restricted and deceased are words, not icons a reader has to know. Photos carry empty alt because the adjacent name is the label." },
      { label: "Name-alike announced", detail: "The collision warning is in the accessible name, not conveyed by styling alone." },
    ],
    limitations: ["Name-alike detection is list-level; the component renders the warning but cannot detect the collision itself.", "Age precision uses birthDate only \\u2014 sub-day precision for neonates needs a birth time the resource rarely carries.", "Initial derivation is Latin-script-biased for multi-word names, though single tokens work in any script."],
  ...
  },
  {
    ...
    categories: ["Primitive", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Coded concept", "Text only, no coding", "Multiple codings", "Unrecognised system", "Outside expected value set", "No concept recorded"],
    props: [
      { name: "concept", type: "CodeableConcept", description: "The concept to display." },
      { name: "expectedCodes", type: "string[]", description: "Codes the concept should carry, as \u201csystem|code\u201d or bare \u201ccode\u201d. Anything outside is flagged." },
      { name: "showSystem", type: "boolean", default: "false", description: "Show the terminology name inline, e.g. \u201cSNOMED CT \u00b7 Hypertension\u201d." },
      { name: "readOnly", type: "boolean", default: "false", description: "Render as plain text with no disclosure. For dense grids." },
      { name: "size", type: "\"sm\" | \"md\"", default: "\"sm\"", description: "Chip size." },
      { name: "field", type: "string", description: "Field name for the accessible name when the chip stands alone." },
    ],
...
<ConceptChip concept={concept} expectedCodes={["http://loinc.org|2823-3"]} />`,
    guidance: {
      use: ["Problem lists, order details, and anywhere a coded concept is shown to a clinician.", "With expectedCodes on surfaces where mapping quality matters.", "With readOnly in virtualised grids, where a popover per row is unusable."],
      avoid: ["Rendering the system URI as though it were a friendly name. Unrecognised means unrecognised.", "Using it for concepts you never intend a user to inspect \\u2014 readOnly text is cheaper.", "Assuming a coded concept is valid because it renders. The chip shows what is there."],
    },
    accessibility: [
      { label: "Button, not hover", detail: "The disclosure is a real button with an expanded state, dismissible with Escape. Nothing is hover-only." },
      { label: "Code out of the name", detail: "The accessible name carries the concept text and any warning; system and code live inside the disclosure rather than crowding it." },
      { label: "Warnings in text", detail: "Text-only and out-of-value-set are labelled in words, never by colour alone." },
    ],
    limitations: ["No terminology server lookup \\u2014 it displays what the payload carries.", "Post-coordinated SNOMED expressions render as their raw code.", "The recognised-system list is finite; extend it as your integrations grow."],
  ...
  },
  {
    ...
    states: ["Patient density", "Standard density", "Clinical density", "Nested providers", "Target-size floor enforced"],
    props: [
      { name: "density", type: "\"patient\" | \"standard\" | \"clinical\"", description: "The density in force for this subtree." },
      { name: "asChild", type: "boolean", default: "false", description: "Clone the single child instead of wrapping. For table rows where an extra div breaks layout." },
...
const density = useDensity();`,
    guidance: {
      use: ["At the root of a worklist, flowsheet, or patient-facing surface.", "Nested, when a patient-facing card sits inside a clinical screen.", "With asChild inside table rows and other layout-sensitive containers."],
      avoid: ["Deriving density from the viewport. Density and breakpoint are independent axes.", "Using clinical density to fit more facts by hiding some. That is a defect.", "Assuming the floor makes any spacing safe \\u2014 it clamps targets, not text size."],
    },
    accessibility: [
      { label: "Target-size floor", detail: "Clinical density cannot shrink an interactive target below the WCAG 2.2 minimum of 24 CSS pixels. DensityTarget enforces it around any control." },
      { label: "Independent of zoom", detail: "Density is an author decision; user text scaling and zoom apply on top and are not overridden." },
      { label: "No content hidden", detail: "Spacing changes only. Nothing that is visible at patient density disappears at clinical density." },
    ],
    limitations: ["Does not read OS accessibility preferences \\u2014 wire those into the density you pass.", "The floor applies to controls wrapped in DensityTarget, not automatically to every descendant.", "Token values come from @oxygenui/tokens; this sets the attribute and context, not the spacing scale."],
  ...
  },
  {
    ...
    dependencies: ["lucide-react", "clsx", "tailwind-merge"],
    states: ["Restricted, disclosure permitted", "Restricted, disclosure not permitted", "Existence concealed", "Disclosed with countdown", "Auto re-redacted", "Not restricted"],
    props: [
      { name: "restricted", type: "boolean", default: "false", description: "Whether the content is restricted. Decided by the application, not here." },
      { name: "disclosurePermitted", type: "boolean", default: "true", description: "Whether this viewer may break the glass. False renders no disclosure path and explains why." },
      { name: "concealExistence", type: "boolean", default: "false", description: "Do not acknowledge that content exists at all." },
      { name: "category", type: "string", default: "\"Restricted record\"", description: "Category shown to the reader, e.g. \u201cSubstance use \u2014 42 CFR Part 2\u201d." },
      { name: "reasons", type: "string[]", description: "Reasons a viewer may choose from. Free text alone is not auditable." },
      { name: "durationSeconds", type: "number", default: "300", description: "How long a disclosure lasts before the shield closes again." },
      { name: "onDisclose", type: "(event: DisclosureEvent) => void", description: "Emitted on disclosure. The application persists it \u2014 the component does not." },
    ],
...
>
  <ObservationPanel observations={toxicology} />
    ...
    guidance: {
      use: ["Around any content subject to a sensitivity policy, at field, section, or record level.", "With onDisclose wired to your audit store \\u2014 the event is emitted, not saved.", "With concealExistence only where the existence of the record is itself sensitive."],
      avoid: ["Treating it as access control. Restricted data still reached the browser; enforce server-side.", "Long durationSeconds values. A disclosure that lasts until logout is not time-boxed.", "Putting the sensitive category name in a category string that itself reveals the content."],
    },
    accessibility: [
      { label: "Announced as withheld", detail: "The redacted state reads as restricted content with an available action, never as an empty section." },
      { label: "Polite countdown", detail: "The re-hide timer uses a polite live region. A countdown that interrupts every second is worse than the risk it mitigates." },
      { label: "Real form controls", detail: "The reason is a labelled select, not a free-text box, so the disclosure record is auditable." },
    ],
    limitations: ["Renders policy outcomes; it does not evaluate consent, security labels, or role.", "The audit event is emitted only. Persistence, retention, and review are yours.", "Timer state is per-instance and resets on remount."],
  ...
  },
  {
    ...
    categories: ["Primitive", "System"],
    dependencies: ["lucide-react", "clsx", "tailwind-merge"],
    states: ["Reversible confirm", "Irreversible type-to-confirm", "Hold-to-confirm", "Reason required", "In flight", "Cancelled"],
    props: [
      { name: "action", type: "string", description: "Imperative name. The same word appears on the trigger, in the dialog, and in the result." },
      { name: "consequence", type: "string", description: "What actually happens, in plain language. Never \u201cAre you sure?\u201d." },
      { name: "patientName", type: "string", description: "The patient affected. Acting on the wrong chart is the error being prevented." },
      { name: "reversible", type: "boolean", default: "true", description: "Drives the default friction level and the closing sentence." },
      { name: "mode", type: "\"confirm\" | \"type\" | \"hold\"", description: "Escalate friction. Defaults to confirm when reversible, type when not." },
      { name: "reasons", type: "string[]", description: "Require a reason before proceeding. Recorded on the audit event." },
      { name: "onConfirm", type: "(event: ActionAuditEvent) => void | Promise<void>", description: "Emitted on confirmation. Persisting the event is the application's job." },
    ],
...
/>`,
    guidance: {
      use: ["Anything that changes a clinical record, medication, schedule commitment, or consent state.", "With reversible={false} and mode=\\u201ctype\\u201d for genuinely irreversible actions.", "With mode=\\u201chold\\u201d on dense screens where a stray double-click could commit."],
      avoid: ["Generic consequence text. If it does not name the effect, it is not a confirmation.", "Gating routine, reversible actions \\u2014 friction everywhere teaches people to click through.", "Assuming the audit event is stored. It is emitted; you persist it."],
    },
    accessibility: [
      { label: "Focus managed", detail: "Focus moves into the dialog on open and returns to the trigger on dismiss. Escape always exits." },
      { label: "Consequence in the description", detail: "The dialog is an alertdialog whose accessible description carries the consequence and the patient name, not just visible text." },
      { label: "Hold has a keyboard path", detail: "Hold-to-confirm works with Space and Enter, so it is never pointer-only." },
    ],
    limitations: ["Emits an audit event; it does not persist one or guarantee delivery.", "No re-authentication step \\u2014 compose one around it where policy requires.", "Focus is managed but not fully trapped; a portal-based dialog is the next step."],
  ...
  },
  {
    ...
    categories: ["Primitive", "System"],
    dependencies: ["lucide-react", "clsx", "tailwind-merge"],
    states: ["Never recorded", "Filtered to empty", "Source unavailable", "Restricted", "Pending", "Compact"],
    props: [
      { name: "reason", type: "\"never\" | \"filtered\" | \"unavailable\" | \"restricted\" | \"pending\"", default: "\"never\"", description: "Why there is nothing here. These are not interchangeable." },
      { name: "title", type: "string", description: "Required. There is no safe default \u2014 a wrong sentence asserts a clinical negative." },
      { name: "description", type: "string", description: "Optional second line explaining what to do." },
      { name: "lastCheckedLabel", type: "string", description: "When the source was last read successfully. Without it, an empty list looks current." },
      { name: "action", type: "ReactNode", description: "The next step, where there is one." },
      { name: "compact", type: "boolean", default: "false", description: "Single-line rendering for table cells and clinical density." },
    ],
...
<EmptyState reason="unavailable" title="Allergies could not be loaded" lastCheckedLabel="Last read 08:41" />`,
    guidance: {
      use: ["Every list, table, and section that can render nothing.", "With reason=\\u201cunavailable\\u201d whenever a fetch failed, never \\u201cnever\\u201d.", "With lastCheckedLabel wherever staleness would change a clinical reading."],
      avoid: ["Asserting a clinical negative. \\u201cNo allergies recorded\\u201d is safe; \\u201cNo allergies\\u201d is a claim.", "Reusing one empty state for a failed fetch and a genuinely empty list.", "Illustration-led empty states on clinical surfaces \\u2014 the sentence is the content."],
    },
    accessibility: [
      { label: "Filtered emptiness announced", detail: "Emptiness that follows a user action uses a polite live region; emptiness that was always there does not, because it is not news." },
      { label: "Icon decorative", detail: "The glyph is hidden from assistive technology. The message carries the meaning." },
      { label: "Reason exposed", detail: "data-empty-reason is on the element for testing and for styling without re-deriving state." },
    ],
    limitations: ["Copy is yours \\u2014 the component enforces the distinction, not the wording.", "No retry behaviour built in; pass an action.", "Does not detect its own reason; the caller knows why the section is empty."],
  ...
  },
  {
    ...
    props: [
      ...
      { name: "variant", type: "\"text\" | \"row\" | \"card\"", default: "\"text\"", description: "Match the shape of the content being replaced, to avoid layout shift." },
      { name: "state", type: "\"loading\" | \"loaded\" | \"failed\" | \"stale\"", description: "ProgressiveSection: the one honest state this section is in." },
      { name: "label", type: "string", description: "ProgressiveSection: section name, used in the failure message." },
      { name: "cachedAtLabel", type: "string", description: "ProgressiveSection: when cached content shown while revalidating was read." },
      { name: "onRetry", type: "() => void", description: "ProgressiveSection: retry handler for the failed state." },
    ],
...
>
  <MedicationList requests={medications.data} />
    ...
    guidance: {
      use: ["Around every independently-fetched section on a composed clinical screen.", "With one section per source system, so one slow service does not block the rest.", "With state=\\u201cstale\\u201d whenever cached content is shown during revalidation."],
      avoid: ["Skeletons shaped like specific values \\u2014 that is the failure this prevents.", "A single page-level spinner for a screen backed by several sources.", "Rendering a failed section as empty. That is the harm ProgressiveSection exists to stop."],
    },
    accessibility: [
      { label: "Announced once", detail: "The skeleton is marked busy and announces once, not on every frame." },
      { label: "Failure is assertive", detail: "A failed section uses role=alert, because it changes what the reader can conclude from the screen." },
      { label: "Reduced motion respected", detail: "Shimmer is motion-safe only; reduced-motion users get a static placeholder." },
    ],
    limitations: ["Does not fetch or retry on its own \\u2014 it renders the state you pass.", "No automatic timeout detection; decide when slow becomes failed.", "Skeleton shapes approximate layout and will not perfectly prevent shift in every composition."],
  ...
  },
  {
    ...
    categories: ["Primitive", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Well-formed dose", "Trailing zero", "Missing leading zero", "Above plausible range", "Above absolute maximum", "Weight-based calculation", "No weight on file"],
    props: [
      { name: "value", type: "string", description: "Current entry, as typed. Kept as a string so precision is not lost." },
      { name: "units", type: "string[]", description: "Units permitted for this drug and route. Never a free list." },
      { name: "plausibleMin / plausibleMax", type: "number", description: "Soft bounds. Crossing them warns with a reason and does not block." },
      { name: "absoluteMax", type: "number", description: "Documented hard maximum. Crossing it blocks and states the maximum." },
      { name: "dosePerKg", type: "number", description: "Enables weight-based calculation." },
      { name: "weightKg", type: "number", description: "Recorded weight. Omit it and no calculation is offered." },
    ],
...
/>`,
    guidance: {
      use: ["Any dose, weight, rate, or volume entry.", "With absoluteMax set from the drug reference, not from a guess.", "With weightKg wired to a recorded weight and left undefined when there is none."],
      avoid: ["Silently rewriting a typed dose. The corrected form is offered, never applied.", "Using plausibleMax as a hard stop \\u2014 unusual doses are sometimes correct.", "Substituting an average weight when none is recorded."],
    },
    accessibility: [
      { label: "Messages bound to the field", detail: "Every warning is associated through aria-describedby and announced on the input, not discovered at submit." },
      { label: "Blocking states are alerts", detail: "Crossing the absolute maximum uses role=alert and sets aria-invalid." },
      { label: "No spinner", detail: "A numeric keypad without increment controls, so a stray scroll cannot change a prescription." },
    ],
    limitations: ["Plausibility bounds are supplied by the caller \\u2014 there is no built-in drug reference.", "Unit lists are caller-supplied; the component does not know which units suit a formulation.", "Override workflow for exceeding the maximum is yours to build; this blocks and explains."],
  ...
  },
  {
    ...
    categories: ["Primitive", "System"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Clinician-entered", "Patient-reported", "Device-recorded", "Interface feed", "Amended", "Source not recorded", "Charted after the event"],
    props: [
      { name: "provenance", type: "Provenance", description: "FHIR Provenance for the datum." },
      { name: "resource", type: "Resource", description: "The resource itself, read for meta.versionId and meta.lastUpdated." },
      { name: "timeZone", type: "string", description: "IANA zone. Required, for the same reason it is on ClinicalTime." },
      { name: "subject", type: "string", description: "What this describes, used in the accessible name." },
      { name: "inline", type: "boolean", default: "false", description: "Render attribution as text instead of behind a disclosure." },
    ],
...
/>`,
    guidance: {
      use: ["Beside any value whose source a clinician might reasonably question.", "On results that can be amended \\u2014 the amended state shows on the trigger.", "With inline on dense surfaces where a popover per row is unusable."],
      avoid: ["Treating an absent provenance as clinician-entered. Unknown is rendered as unknown.", "Hiding amendment behind the disclosure only.", "Assuming meta.lastUpdated is the clinical event time. They are different facts."],
    },
    accessibility: [
      { label: "Named trigger", detail: "The button says what it reveals and about what, rather than being a bare icon." },
      { label: "Dialog semantics", detail: "Content is a labelled group, dismissible with Escape; nothing is hover-only." },
      { label: "Amendment in the name", detail: "The amended state is in the trigger's accessible name, not only in its colour." },
    ],
    limitations: ["Reads one agent and one source entity; complex provenance chains are summarised.", "Does not fetch version history \\u2014 it reports what the payload and meta carry.", "Amendment detection uses versionId and revision entities; systems that populate neither will read as unamended."],
  ...
  },
  {
    ...
    categories: ["Primitive", "System"],
    dependencies: ["lucide-react", "clsx", "tailwind-merge"],
    states: ["Clean", "Saving", "Saved", "Autosave failed", "Prompt on navigation", "Prompt on patient switch", "Browser close guard"],
    props: [
      { name: "surface", type: "DirtySurface", description: "useUnsavedWork: id, description, saveState, and last-saved label for this editor." },
      { name: "confirmLeave", type: "(intent: string) => Promise<boolean>", description: "useConfirmLeave: ask the guard whether it is safe to leave." },
      { name: "state", type: "SaveState", description: "SaveStatus: clean, saving, saved, or failed." },
      { name: "lastSavedLabel", type: "string", description: "SaveStatus: when the last successful save happened." },
    ],
    usage: `import {
...
if (await confirmLeave("Switching to another patient.")) switchPatient();`,
    guidance: {
      use: ["Around the whole application, once, at the root.", "In every editing surface \\u2014 a guard only some forms use is a guard that does not work.", "Before any patient-context change, not just route changes."],
      avoid: ["Describing a surface as \\u201cform\\u201d or \\u201cchanges\\u201d. Name what would be lost.", "Treating a failed autosave as recoverable. That draft exists only in this tab.", "Relying on the browser prompt alone \\u2014 it cannot be styled or worded."],
    },
    accessibility: [
      { label: "Managed focus", detail: "The prompt is an alertdialog; focus moves in on open and Escape cancels the departure." },
      { label: "Failure is assertive", detail: "Autosave failure announces assertively. A silent failure lets someone write for twenty minutes believing their note is filed." },
      { label: "Named stakes", detail: "The dialog lists each dirty surface by description, so the consequence is legible rather than generic." },
    ],
    limitations: ["The browser-level prompt cannot be styled or worded \\u2014 that is a platform limit.", "Does not persist drafts; it reports save state and blocks. Persistence is yours.", "Route interception depends on your router calling confirmLeave."],
  ...
  },
  {
    ...
    props: [
      { name: "label", type: "string", description: "Section name used in the message. \u201cMedications\u201d, not \u201cMedListView\u201d." },
      { name: "critical", type: "boolean", default: "false", description: "Marks a boundary whose failure invalidates the surrounding screen." },
      { name: "onError", type: "(report: ErrorReport) => void", description: "Reported without PHI \u2014 reference, boundary, and message only." },
      { name: "onRetry", type: "() => void", description: "Called when the user retries after a reset." },
    ],
...
<ClinicalErrorBoundary label="Patient banner" critical>
  <PatientBanner patient={patient} />
    ...
    guidance: {
      use: ["Around every independently-rendered clinical section.", "With critical on identity, code status, and allergy surfaces.", "With onError wired to monitoring that you have confirmed scrubs PHI."],
      avoid: ["One boundary around the whole page \\u2014 that is the blank screen this avoids.", "Rendering null on error. There is deliberately no silent path.", "Passing the raw error to a logger without checking what it contains."],
    },
    accessibility: [
      { label: "Assertive", detail: "Failures use role=alert, because they change what the reader can conclude from the screen." },
      { label: "Quotable reference", detail: "A short reference id the user can read aloud to support, with no stack trace or PHI." },
      { label: "Retry is a real control", detail: "Recovery is a labelled button, not a page reload instruction." },
    ],
    limitations: ["React error boundaries do not catch errors in event handlers or async code.", "PHI scrubbing covers what this sends; your monitoring pipeline is still yours to verify.", "Reset re-mounts children; it does not re-fetch unless onRetry does."],
  ...
  },
  {
    ...
    dependencies: ["lucide-react", "clsx", "tailwind-merge"],
    states: ["Role with full access", "Role with limited access", "No features enabled", "Session expiring", "Break-glass active", "Mobile navigation"],
    props: [
      { name: "scopes", type: "string[]", description: "Scopes the user holds, as resolved by the server." },
      { name: "items", type: "NavItem[]", description: "Destinations, each optionally requiring a scope." },
      { name: "patientContext", type: "ReactNode", description: "Persistent patient context, rendered as its own landmark." },
      { name: "onNavigate", type: "(item) => Promise<boolean> | boolean", description: "Called before a destination change so unsaved work can veto it." },
      { name: "sessionSecondsRemaining", type: "number", description: "Under the warning threshold, a countdown and extend control appear." },
      { name: "breakGlassActive", type: "boolean", default: "false", description: "Emergency access in force. Marked persistently, not once at entry." },
    ],
...
>
  {children}
    ...
    guidance: {
      use: ["As the single outermost frame of a clinical application.", "With onNavigate wired to the unsaved-work guard.", "With requires set on every destination that is permission-gated."],
      avoid: ["Treating hidden navigation as access control. The server remains the authority.", "Dropping patient context on narrow screens \\u2014 it compresses, never disappears.", "Rendering permitted-but-disabled items. Absent is clearer and more auditable."],
    },
    accessibility: [
      { label: "Correct landmarks", detail: "Primary navigation, patient context, and main content are separate labelled landmarks." },
      { label: "Counts in the name", detail: "Badge counts are part of each link's accessible name, with urgent distinguished from unread." },
      { label: "Provisioning failure is visible", detail: "A role with nothing permitted says so rather than rendering an empty product." },
    ],
    limitations: ["Does not implement routing \\u2014 it renders links and delegates to onNavigate.", "Session countdown is driven by the prop; the timer itself is yours.", "Multi-patient tab management is a separate component."],
  ...
  },
  {
    ...
    categories: ["Patient identity", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Full code", "DNR", "DNR / DNI", "Comfort measures", "Not on file", "Verification stale", "Conflicting directives"],
    props: [
      { name: "status", type: "ResuscitationStatus", default: "\"unknown\"", description: "Resuscitation status. Defaults to unknown, never to full code." },
      { name: "verifiedAt / verifiedBy", type: "string", description: "When and by whom the status was last confirmed." },
      { name: "staleAfterDays", type: "number", default: "30", description: "Policy window after which verification is flagged as stale." },
      { name: "documents", type: "Array", description: "Directive documents, linked rather than merely referenced." },
      { name: "proxy / proxyPhone", type: "RelatedPerson | string", description: "Healthcare proxy and a tel: contact route." },
      { name: "conflicting", type: "boolean", default: "false", description: "More than one directive on file that do not agree." },
    ],
...
/>`,
    guidance: {
      use: ["High in the reading order of any summary, admission, or emergency surface.", "With verifiedAt always supplied \\u2014 an unverified directive is a different claim.", "With conflicting set when more than one directive exists and they disagree."],
      avoid: ["Defaulting status to full-code when nothing is found. That is a clinical decision.", "Hiding it behind a tab or a disclosure.", "Resolving conflicting directives programmatically."],
    },
    accessibility: [
      { label: "First in reading order", detail: "A labelled section placed early, so it is reached before the content it governs." },
      { label: "Never colour alone", detail: "Status is a full phrase; the DNR states are spelled out rather than abbreviated to a badge." },
      { label: "Reachable proxy", detail: "The proxy phone is a tel: link, usable one-handed during an arrest." },
    ],
    limitations: ["Does not resolve conflicts or rank directives \\u2014 it surfaces them.", "Does not verify document validity across organisations.", "Status vocabulary is a fixed set; jurisdictional variants need mapping upstream."],
  ...
  },
  {
    ...
    categories: ["Patient identity", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Airborne", "Contact isolation", "Behavioral approach", "Fall risk", "Mobility", "No active precautions", "Lapsed and dropped"],
    props: [
      { name: "flags", type: "Flag[]", description: "FHIR Flags. Only those active at asOf are rendered." },
      { name: "precautions", type: "PrecautionDisplay[]", description: "Explicit precautions for callers not modelling these as Flags." },
      { name: "actionFor", type: "(flag) => string | undefined", description: "Maps a Flag to its required action. Without one, the label renders alone." },
      { name: "asOf", type: "Date", default: "new Date()", description: "Evaluation date. Pass a fixed date for deterministic tests." },
    ],
...
/>`,
    guidance: {
      use: ["At the top of any chart, room display, or transport handover.", "With actionFor supplying the specific PPE or approach required.", "On ward and corridor displays, where it is read at distance."],
      avoid: ["Rendering lapsed precautions greyed out rather than removing them.", "Behavioral labels that characterise a person rather than an approach.", "Relying on the icon alone \\u2014 these are read on poor displays at distance."],
    },
    accessibility: [
      { label: "Icon, text, and colour", detail: "All three, always. These are read at distance and often in monochrome." },
      { label: "Ordered by action", detail: "Reading order follows urgency of required action, not the source order of the flags." },
      { label: "Empty is stated", detail: "No active precautions renders as a sentence rather than an empty bar." },
    ],
    limitations: ["PPE mapping is caller-supplied; there is no built-in infection-control catalogue.", "Category mapping covers common codes and falls back to a neutral kind.", "Does not model precaution ordering rules beyond urgency of action."],
  ...
  },
  {
    ...
    categories: ["Patient identity", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["Current team", "Covered member", "Past members", "Non-clinical members", "No team recorded", "Role not recorded"],
    props: [
      { name: "team", type: "CareTeam", description: "FHIR CareTeam resource." },
      { name: "extraMembers", type: "CareTeamMember[]", description: "Members not in the resource, e.g. community supports." },
      { name: "coverage", type: "Record<string, CoverageInfo>", description: "Who is actually reachable for a member right now." },
      { name: "contacts", type: "Record<string, {...}>", description: "Contact routes by member reference or name." },
      { name: "responsibleRef", type: "string", description: "Responsible clinician, pinned first." },
      { name: "showPast", type: "boolean", default: "true", description: "Keep ended memberships available as history." },
    ],
...
/>`,
    guidance: {
      use: ["On any chart where someone might need to contact the team.", "With coverage supplied from the on-call schedule, not the assignment record.", "With community and caregiver members passed through extraMembers."],
      avoid: ["Replacing the assigned clinician with the covering one. Both facts are needed.", "Deleting past members \\u2014 reviews ask who was involved and when.", "Demoting non-clinical members to a footnote."],
    },
    accessibility: [
      { label: "Named actions", detail: "Contact buttons say the action and the person, not just an icon." },
      { label: "Avatars decorative", detail: "Initials are hidden from assistive technology; the name carries meaning." },
      { label: "Coverage is text", detail: "Off-call status is a sentence, not a colour or a dimmed row." },
    ],
    limitations: ["Coverage is caller-supplied \\u2014 there is no on-call schedule resolution here.", "Participants without a display name are skipped rather than shown as unknown.", "Does not model team hierarchy; order is source order with the responsible clinician pinned."],
  ...
  },
  {
    ...
    props: [
      { name: "severity", type: "AlertSeverity", default: "\"info\"", description: "Interruption tier. Only critical takes focus." },
      { name: "finding", type: "string", description: "The specific finding. Never a category like \u201cAbnormal result\u201d." },
      { name: "detail", type: "string", description: "Mechanism or consequence, where it helps a decision." },
      { name: "source", type: "string", description: "Where it came from, so a clinician can judge it." },
      { name: "actions", type: "ReactNode", description: "Inline actions, so responding does not require navigating away." },
      { name: "dismissReasons", type: "string[]", description: "Reasons offered when dismissing a critical alert." },
      { name: "onDismiss", type: "(dismissal) => void", description: "Emitted with the reason, for alert-performance measurement." },
    ],
...
/>`,
    guidance: {
      use: ["Anywhere a clinical finding must be seen before the user proceeds.", "With the specific value and interpretation in finding.", "With dismissReasons on critical alerts, so rule performance is measurable."],
      avoid: ["Declaring everything critical. The tier is a budget and it is finite.", "Category text like \\u201cAbnormal result\\u201d \\u2014 it teaches dismissal without reading.", "Dismissal with no reason on a critical alert; that is how bad rules survive."],
    },
    accessibility: [
      { label: "Tiered live regions", detail: "Only critical is assertive. Everything else is polite and waits its turn in the reading order." },
      { label: "Severity as a word", detail: "The tier is in the accessible name, so it survives greyscale and forced-colors." },
      { label: "Named dismiss", detail: "The dismiss control names the alert it closes rather than being a bare X." },
    ],
    limitations: ["Does not deduplicate or rank multiple simultaneous alerts \\u2014 compose a stack.", "Suppression across encounters is application state, not component state.", "Emits dismissal reasons; storing and analysing them is yours."],
  ...
  },
  {
    ...
    categories: ["Patient identity", "Clinical"],
    dependencies: ["@oxygenui/fhir", "lucide-react", "clsx", "tailwind-merge"],
    states: ["All sections loaded", "Section failed", "Section stale", "Section empty", "Truncated with a count", "Changes since last review"],
    props: [
      { name: "sections", type: "SnapshotSection[]", description: "Each with its own state, counts, recency, and empty copy." },
      { name: "timeZone", type: "string", description: "IANA zone for every timestamp on the summary." },
      { name: "lastReviewedAt", type: "string", description: "When this reader last reviewed the chart, for the change summary." },
      { name: "columns", type: "1 | 2", default: "2", description: "Two columns on desktop, or a single prioritised stack." },
    ],
...
/>`,
    guidance: {
      use: ["Pre-visit and handover surfaces, where the reader has minutes rather than an hour.", "With one section per source system, so one slow service does not block the rest.", "With totalCount and shownCount whenever the section is truncated."],
      avoid: ["Rendering a failed section as empty. That is the harm this exists to prevent.", "Truncating without a count.", "Making it exhaustive. A summary that shows everything is not a summary."],
    },
    accessibility: [
      { label: "Sections as regions", detail: "Each section is a labelled article, so a screen-reader user navigates section by section." },
      { label: "Failure summarised first", detail: "Failed sections are announced at the top, before the reader forms a picture from the sections above them." },
      { label: "Change counts in the name", detail: "New-since-review counts are part of the heading, not a decorative badge." },
    ],
    limitations: ["Does not decide what belongs in a summary \\u2014 sections and priority are yours.", "Specialty configuration is caller-supplied.", "Change detection compares counts you supply; it does not diff resources."],
