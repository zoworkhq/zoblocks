 ...
 * **bridge** writes that framework's resolved theme onto `--ox-*`, so the
 * Oxygen components inside restyle without importing anything. A reader who
 * switches to Material UI gets MUI's real Button — ripple and all — beside an
 * Oxygen ResultValue wearing MUI's palette, which is what a customer's screen
 * actually looks like.
 *
 ...
 * Suspense fallback can render the same children under the Oxygen host, so the
 ...
 * `"oxygen"` during SSR and the first client render — see `design-language.ts`
...
import { OxygenHost } from "@oxygenui-design/host-react";
...
const AntdHost = React.lazy(async () => {
  const loaded = await import("@oxygenui-design/host-react/antd");
  return { default: loaded.AntdHost };
});

const MuiHost = React.lazy(async () => {
  const loaded = await import("@oxygenui-design/host-react/mui");
...
export function HostStage({
  ...
  /*
   * The fallback is the Oxygen host, not a spinner.
   *
   ...
   */
  const fallback = (
    <OxygenHost mode={mode} className={className}>
      {children}
    </OxygenHost>
  );
