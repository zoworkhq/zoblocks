 ...
 * `host-react`: the Signature demo, because `@oxygenui-design/signature`
 ...
 * Oxygen's accent, which was right when Oxygen was the only design language
 ...
 * `--ox-accent` back out of the DOM, which would also mean a computed-style
...
const BRAND: Record<DesignLanguage, { light: AntdBrandTokens; dark: AntdBrandTokens }> = {
  oxygen: {
    // `colorTextLightSolid` moves with the accent because Oxygen's dark accent
  ...
  mui: {
    ...
    // writes to `--ox-accent` and `--ox-text-on-accent`.
