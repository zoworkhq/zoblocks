...
export const metadata: Metadata = {
  title: "Compare — Oxygen UI, antd, or build it",
  description:
    "How Oxygen UI compares to general-purpose React libraries, headless table libraries, and building clinical components in-house. Measured, with sources.",
              ...
              {OPTIONS.map((option) => (
                <div key={option.name} className="bg-paper p-6 sm:p-8" data-reveal>
                  <div className="flex flex-wrap items-baseline justify-between gap-3">
                    ...
                    <p className="numeric text-xs text-oxygen-deep">{option.cost}</p>
                  </div>
                    ...
                    <tr key={row.claim} className="border-b border-rule last:border-b-0">
                      ...
                      <td
                        className={`px-5 py-3.5 align-top text-sm ${
                          row.tone === "gap" ? "text-critical" : "text-oxygen-deep"
                        }`}
                      >
                        {row.finding}
                      </td>
