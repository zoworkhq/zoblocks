 ...
 * reading `--ox-accent` back out of the DOM is not an option.
 *
 ...
 * what `muiBridge.map()` writes for `--ox-accent` and `--ox-text-on-accent`
 * from an untouched `createTheme()`, and the Oxygen values are the resolved
 * `--ox-accent` and `--ox-text-on-accent` from `packages/tokens`. If either
...
describe("the antd brand table", () => {
  it("gives Oxygen its own accent, in both modes", () => {
    // `--ox-accent` / `--ox-text-on-accent`, light and dark, from
    // packages/tokens/src/oxygen-tokens.css.
    expect(antdBrandTokens("oxygen", false)).toEqual({
      colorPrimary: "#067662",
      colorTextLightSolid: "#ffffff",
    });
    expect(antdBrandTokens("oxygen", true)).toEqual({
      colorPrimary: "#6ce7cb",
  ...
  it("covers every language the switcher offers", () => {
    ...
    for (const language of ["oxygen", "antd", "mui"] as const) {
