  ...
  return (
    <>
      <button
        ...
        className={cn(
          "inline-flex items-center gap-1.5 rounded-[var(--zb-radius)] border px-3 py-1.5 text-[length:var(--zb-text-sm)] font-semibold",
          destructive
            ? "border-[var(--zb-status-critical)] text-[var(--zb-status-critical)] hover:bg-[var(--zb-status-critical-bg)]"
            : "border-[var(--zb-border-strong)] text-[var(--zb-text)] hover:bg-[var(--zb-bg-muted)]",
          className,
        )}
      >
        {children ?? action}
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
          <div
            ...
            role="alertdialog"
            aria-modal="true"
            aria-labelledby="zb-gate-title"
            aria-describedby="zb-gate-desc"
            className="w-full max-w-md rounded-[var(--zb-radius-lg)] border border-[var(--zb-border)] bg-[var(--zb-surface-overlay)] p-5 shadow-[var(--zb-shadow-lg)]"
          >
            <h2
              id="zb-gate-title"
              className="flex items-center gap-2 text-[length:var(--zb-text-lg)] font-bold"
            >
              {!reversible && (
                <TriangleAlert
                  aria-hidden="true"
                  className="size-4 text-[var(--zb-status-critical)]"
                />
              )}
              {action}
            </h2>
            ...
            <p
              id="zb-gate-desc"
              className="mt-2 text-[length:var(--zb-text-sm)] leading-relaxed text-[var(--zb-text-muted)]"
            >
              {consequence}
              {patientName && (
                <>
                  {" "}
                  This affects <strong className="text-[var(--zb-text)]">{patientName}</strong>.
                </>
              )}{" "}
            ...
            {reasons?.length ? (
              <div className="mt-4 flex flex-col gap-1">
                <label
                  htmlFor="zb-gate-reason"
                  className="text-[length:var(--zb-text-2xs)] font-semibold uppercase tracking-wider text-[var(--zb-text-muted)]"
                >
                  Reason
                </label>
                <select
                  ...
                  className="rounded-[var(--zb-radius-sm)] border border-[var(--zb-border-strong)] bg-[var(--zb-surface)] px-2 py-1.5 text-[length:var(--zb-text-sm)]"
                >
            ...
            {resolvedMode === "type" && (
              <div className="mt-4 flex flex-col gap-1">
                <label
                  htmlFor="zb-gate-type"
                  className="text-[length:var(--zb-text-2xs)] font-semibold uppercase tracking-wider text-[var(--zb-text-muted)]"
                >
                  Type <span className="font-[family-name:var(--zb-font-mono)]">{phrase}</span> to
                  confirm
                </label>
                <input
                  id="zb-gate-type"
                  ...
                  className="rounded-[var(--zb-radius-sm)] border border-[var(--zb-border-strong)] bg-[var(--zb-surface)] px-2 py-1.5 font-[family-name:var(--zb-font-mono)] text-[length:var(--zb-text-sm)]"
                />
              </div>
            )}
            ...
            <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button
                type="button"
                onClick={close}
                className="rounded-[var(--zb-radius)] border border-[var(--zb-border-strong)] px-3 py-2 text-[length:var(--zb-text-sm)] font-semibold hover:bg-[var(--zb-bg-muted)]"
              >
                Cancel
              </button>
              ...
              {resolvedMode === "hold" ? (
                <button
                  ...
                  onKeyUp={endHold}
                  className="relative overflow-hidden rounded-[var(--zb-radius)] bg-[var(--zb-status-critical)] px-3 py-2 text-[length:var(--zb-text-sm)] font-semibold text-white disabled:opacity-50"
                >
                  <span
                    aria-hidden="true"
                ...
                <button
                  ...
                  onClick={commit}
                  className={cn(
                    "rounded-[var(--zb-radius)] px-3 py-2 text-[length:var(--zb-text-sm)] font-semibold text-white disabled:opacity-50",
                    destructive || !reversible
                      ? "bg-[var(--zb-status-critical)]"
                      : "bg-[var(--zb-accent)]",
                  )}
                >
                  {busy ? "Working…" : action}
            ...
            <p className="mt-3 text-[length:var(--zb-text-2xs)] text-[var(--zb-text-subtle)]">
