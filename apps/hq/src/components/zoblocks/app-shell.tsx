...
export function AppShell({
  ...
  return (
    <div className={cn("flex min-h-full flex-col bg-[var(--zb-bg)]", className)}>
      ...
      {breakGlassActive && (
        <div
          role="status"
          aria-live="polite"
          className="flex items-center justify-center gap-2 bg-[var(--zb-flag-restricted)] px-3 py-1.5 text-[length:var(--zb-text-xs)] font-semibold text-white"
        >
      ...
      {expiring && (
        <div
          role="alert"
          className="flex flex-wrap items-center justify-center gap-3 bg-[var(--zb-status-high-bg)] px-3 py-1.5 text-[length:var(--zb-text-xs)] text-[var(--zb-status-high)]"
        >
          ...
          {onExtendSession && (
            <button
              ...
              className="rounded-[var(--zb-radius-sm)] border border-[var(--zb-status-high)] px-2 py-0.5 font-semibold hover:bg-[var(--zb-surface)]"
            >
              Stay signed in
            </button>
          )}
      ...
      <header className="flex items-center gap-3 border-b border-[var(--zb-border)] bg-[var(--zb-surface)] px-[var(--zb-density-pad-x)] py-[var(--zb-density-pad-y)]">
        <button
          type="button"
          onClick={() => setNavOpen((v) => !v)}
          aria-expanded={navOpen}
          aria-controls="zb-shell-nav"
          aria-label={navOpen ? "Close navigation" : "Open navigation"}
          className="rounded-[var(--zb-radius-sm)] border border-[var(--zb-border)] p-1.5 md:hidden"
        >
      ...
      */}
      {patientContext && (
        <section
          ...
          className="border-b border-[var(--zb-border)] bg-[var(--zb-surface)]"
        >
          {patientContext}
        </section>
      )}
      ...
      <div className="flex min-h-0 flex-1">
        <nav
          ...
          aria-label="Primary"
          className={cn(
            "shrink-0 border-r border-[var(--zb-border)] bg-[var(--zb-bg-subtle)] p-2",
            "md:block md:w-56",
        ...
        >
              ...
              return (
                <li key={item.id}>
                  <a
                    ...
                    className={cn(
                      "flex items-center gap-2 rounded-[var(--zb-radius-sm)] px-2 py-1.5 text-[length:var(--zb-density-font)] no-underline",
                      active
                        ? "bg-[var(--zb-accent-subtle)] font-semibold text-[var(--zb-accent)]"
                        : "text-[var(--zb-text-muted)] hover:bg-[var(--zb-bg-muted)] hover:text-[var(--zb-text)]",
                    )}
                  >
                    ...
                    {item.badgeCount ? (
                      <span
                        className={cn(
                          "rounded-[var(--zb-radius-full)] px-1.5 py-0.5 font-[family-name:var(--zb-font-numeric)] text-[length:var(--zb-text-2xs)] font-bold tabular-nums",
                          item.urgent
                            ? "bg-[var(--zb-status-critical)] text-white"
                            : "bg-[var(--zb-bg-muted)] text-[var(--zb-text-muted)]",
                        )}
                      >
          ...
          {permitted.length === 0 && (
            <p className="px-2 py-3 text-[length:var(--zb-text-xs)] text-[var(--zb-text-subtle)]">
        ...
        <main className="min-w-0 flex-1 p-[var(--zb-density-pad-x)]">{children}</main>
      </div>
    </div>
  );
}
