---
"@oxygenui-design/host-react": minor
---

`@oxygenui-design/host-react` — the chrome an application puts around a clinical
...
Oxygen ships 28 components and none of them is a Button. That is ADR 0010
...
- **Clinical colours are still refused.** Each host mounts its token bridge, and
  `bridge-core` throws if one ever writes `--ox-status-*`.
