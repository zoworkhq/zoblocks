...
Move `PatientChip` off `.ox-chip`, which Accordion already owns.

`.ox-chip` is Accordion's severity badge. Any page loading both stylesheets gave
...
staircase. The class is now `.ox-patient-chip`; anyone who styled the old name
...
escalation animate rather than snap. Set `--ox-row` on a row wrapper to stagger.
