---
"@oxygenui-design/cli": minor
---

First release: Oxygen installs its own components.

`npx @oxygenui-design/cli add pulse-loader` replaces the third-party CLI the
...
`oxygenui.design`, built into the binary.

Paid components keep their namespace and bearer token, declared in `oxygen.json`
(which replaces `components.json`). Credentials are written as `${OXYGEN_TOKEN}`
...
`oxygen:*` file kinds and a `$schema` under `oxygenui.design`, so the old CLI
