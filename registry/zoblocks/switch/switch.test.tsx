...
import {
  ...
  type CommitPhase,
  type SwitchValue,
} from "@/lib/zoblocks-switch";
import type { AbsentReason as FhirAbsentReason } from "@zoblocks/fhir";
import {
...
const root = (container: HTMLElement) => container.querySelector<HTMLElement>("[data-zb-switch]");
...
describe("semantics", () => {
  ...
  it("forwards a ref and accepts className, style, id and data attributes", () => {
    ...
    expect(root(view.container)).toHaveClass("zb-switch", "custom");
...
describe("value", () => {
  ...
  it("is controlled when checked is supplied — the caller owns the value", async () => {
    ...
    await waitFor(
      () => {
        expect(root(view.container)).toHaveAttribute("data-zb-phase", "idle");
      },
      { timeout: 2000 },
    );
    expect(control()).toHaveAttribute("aria-checked", "false");
  ...
  it("keeps the absent-reason union in step with @zoblocks/fhir", () => {
...
describe("commit", () => {
  ...
  it("renders the REQUESTED value while pending, never the record's", async () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-phase", "pending");
    expect(root(view.container)).toHaveAttribute("data-zb-state", "on");
    expect(control()).toHaveAttribute("aria-checked", "true");
  });
  ...
  it("holds pending for minPendingMs so a fast write is perceptible", async () => {
    ...
    await advance(220);
    expect(root(view.container)).toHaveAttribute("data-zb-phase", "committed");
  });
  ...
  it("reverts to the record's value and announces the failure assertively", async () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-phase", "reverted");
    expect(root(view.container)).toHaveAttribute("data-zb-state", "off");
    expect(control()).toHaveAttribute("aria-checked", "false");
  ...
  it("distinguishes blocked from reverted", async () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-phase", "blocked");
...
describe("queued", () => {
  ...
  it("queues rather than sending, and says so", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-phase", "queued");
...
describe("stale", () => {
  it("enters stale when the record disagrees with what is on screen", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-phase", "stale");
...
describe("availability", () => {
  it("keeps a read-only switch in the tab order with its reason exposed", async () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-readonly", "true");
  ...
  it("marks a disabled switch aria-disabled and refuses to commit", async () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-disabled", "true");
  });
...
describe("appearances", () => {
  ...
  it("segmented shows the unasked cell as a non-interactive indicator", () => {
    ...
    const unasked = view.container.querySelector(".zb-switch__segment--unasked");
  ...
  it("segmented drops the unasked cell once an answer exists, with no path back", async () => {
    ...
    expect(view.container.querySelector(".zb-switch__segment--unasked")).toBeNull();
  ...
  it("labeled renders both words so the track cannot resize as it toggles", () => {
    ...
    const words = view.container.querySelectorAll(".zb-switch__word");
  ...
  it("checkedChildren implies the labeled appearance, as antd users expect", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-appearance", "labeled");
  ...
  it("chip uses the label as its own text and hides the separate state word", () => {
    ...
    expect(view.container.querySelector(".zb-switch__state")).toBeNull();
  });
  ...
  it("row puts the label and description inside the target", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-appearance", "row");
  });
});
...
describe("presentation", () => {
  ...
  it("never writes a hit-area override, so the density token governs the target", () => {
    ...
    expect(root(view.container)!.style.getPropertyValue("--zb-switch-target-min")).toBe("");
  });
  ...
  it.each(["affirmative", "neutral", "caution", "critical"] as const)(
    ...
    (tone) => {
      ...
      expect(root(view.container)).toHaveAttribute("data-zb-tone", tone);
    },
  );
  ...
  it("preserves a caller's inline custom properties alongside the size vars", () => {
    const view = render(
      <Switch
        label="NPO"
        checked
        style={{ "--zb-switch-track-w": "64px" } as React.CSSProperties}
      />,
    );
    expect(root(view.container)!.style.getPropertyValue("--zb-switch-track-w")).toBe("64px");
  });
  ...
  it("patient audience defaults to a larger control and plainer words", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-size", "large");
    expect(root(view.container)).toHaveAttribute("data-zb-audience", "patient");
    expect(view.container.querySelector(".zb-switch__state")).toHaveTextContent("Yes");
  });
});
...
describe("until", () => {
  ...
  it("renders the window beside the state", () => {
    ...
    expect(view.container.querySelector(".zb-switch__until")?.textContent).toMatch(/until/i);
  ...
  it("derives the lapse from the supplied clock, not the wall clock", () => {
    ...
    expect(view.container.querySelector(".zb-switch__until--lapsed")).not.toBeNull();
  ...
  it("renders nothing about a window while the value is off", () => {
    ...
    expect(view.container.querySelector(".zb-switch__until")).toBeNull();
  });
...
describe("SwitchField and SwitchList", () => {
  ...
  it("omits the summary when there is nothing to count", () => {
    ...
    expect(view.container.querySelector(".zb-switch-list__summary")).toBeNull();
  });
...
describe("Ant Design parity", () => {
   ...
   * `from "@zoblocks/react"` with no other diff. A rename in an antd
  ...
  it("uses antd's small/default size names", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-size", "small");
  });
...
describe("edge paths", () => {
  ...
  describe("stale", () => {
    ...
    it("leaves the conflict alone while a write of your own is in flight", async () => {
      ...
      await user.click(control());
      await waitFor(() => expect(root(view.container)).toHaveAttribute("data-zb-phase", "pending"));
  ...
  describe("minimal props", () => {
    ...
    it("puts the text before the control when asked", () => {
      ...
      const text = rootEl.querySelector(".zb-switch__text") as HTMLElement;
      const group = rootEl.querySelector(".zb-switch__segments") as HTMLElement;
    ...
    it("renders a switch with no text column at all", () => {
      ...
      expect(view.container.querySelector(".zb-switch__text")).toBeNull();
    ...
    it("renders a list with no title and no counts", () => {
      ...
      expect(view.container.querySelector(".zb-switch-list__head")).toBeNull();
      expect(screen.getByRole("switch")).toBeInTheDocument();
    });
...
describe("elapsed time", () => {
  it("degrades to no minimum rather than throwing where performance is absent", async () => {
    ...
    try {
      const { elapsedClock } = await import("@/lib/zoblocks-switch");
...
describe("remaining corners", () => {
  ...
  it("renders an off chip without a glyph", () => {
    ...
    expect(view.container.querySelector(".zb-switch__chip .zb-switch__glyph")).toBeNull();
  });

  it("renders a row with no description", () => {
    const view = render(<Switch label="Text me my results" appearance="row" checked />);
    expect(view.container.querySelector(".zb-switch__desc")).toBeNull();
...
describe("partial composition", () => {
  it("renders a row described but not labelled", () => {
    ...
    expect(view.container.querySelector(".zb-switch__label")).toBeNull();
  ...
  it("renders a list summary without a title", () => {
    ...
    expect(view.container.querySelector(".zb-switch-list__title")).toBeNull();
...
describe("controlled phase", () => {
  ...
  it("covers every phase without throwing", () => {
    ...
    for (const phase of phases) {
      ...
      expect(root(view.container)).toHaveAttribute("data-zb-phase", phase);
      expectNoLeakedValues(view);
      view.unmount();
    }
  });
