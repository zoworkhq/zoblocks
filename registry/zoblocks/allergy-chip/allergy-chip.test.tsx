...
describe("criticality and reaction severity are different facts", () => {
  it("renders both, as different kinds of thing", () => {
    ...
    expect(
      container.querySelector("[data-zb-scale='criticality'][data-zb-step='critical']"),
    ).toBeTruthy();
    expect(container.querySelector(".zb-allergy__manifestation")).toHaveTextContent("Urticaria");
    expect(container.querySelector(".zb-allergy__severity")).toHaveTextContent("mild");
  });
  ...
  it("keeps them apart when they disagree, which is the whole point", () => {
    ...
    const highChip = high.container.querySelector("[data-zb-allergy]")!;
    high.unmount();

    const low = render(<AllergyChip record={amoxicillin} />);
    const lowChip = low.container.querySelector("[data-zb-allergy]")!;

    expect(highChip.getAttribute("data-zb-criticality")).toBe("high");
    expect(lowChip.getAttribute("data-zb-criticality")).toBe("low");
  ...
  it("maps high criticality to critical, not to high", () => {
    ...
    const { container } = render(<AllergyChip record={penicillin} />);
    expect(container.querySelector("[data-zb-step='critical']")).toBeTruthy();
  });
...
describe("the two absences", () => {
  ...
  it("distinguishes the two by shape, not only by colour", () => {
    ...
    expect(asserted.container.querySelector(".zb-allergy-none")).toBeTruthy();
    asserted.unmount();

    const unasked = render(<AllergyList />);
    expect(unasked.container.querySelector(".zb-allergy-unknown")).toBeTruthy();
  });
...
describe("kinds", () => {
  it("renders an intolerance as its own class", () => {
    ...
    expect(container.querySelector(".zb-allergy__kind")).toHaveTextContent("Intolerance");
  ...
  it("leaves the common case unlabelled so the exceptions carry weight", () => {
    const { container } = render(<AllergyChip record={penicillin} />);
    expect(container.querySelector(".zb-allergy__kind")).toBeNull();
  });
});
...
describe("verification", () => {
  ...
  it("keeps a refuted entry readable rather than deleting it", () => {
    ...
    const chip = container.querySelector("[data-zb-allergy]")!;
    expect(chip.hasAttribute("data-zb-inactive")).toBe(true);
...
describe("class expansion", () => {
  ...
  it("renders nothing when the host has no expansion", () => {
    ...
    expect(container.querySelector(".zb-allergy__class")).toBeNull();
  });
});
...
describe("interaction", () => {
  ...
  it("renders every record in a list", () => {
    ...
    expect(container.querySelectorAll("[data-zb-allergy]")).toHaveLength(2);
    const list = container.querySelector("[data-zb-allergy-list]")!;
    expect(list.getAttribute("data-zb-allergy-list")).toBe("records");
