...
describe("the value line", () => {
  ...
  it("never colours the number itself by interpretation", () => {
    ...
    const number = container.querySelector(".zb-rv__number") as HTMLElement;
    expect(number.style.color).toBe("");
    expect(container.querySelector("[data-zb-status][data-zb-scale='criticality']")).toBeTruthy();
  });
});
...
describe("status is never implicit", () => {
  it("renders the status chip whenever a status is known", () => {
    ...
    expect(
      container.querySelector("[data-zb-scale='result-status'][data-zb-step='preliminary']"),
    ).toBeTruthy();
  });
...
describe("a correction shows the value it replaced", () => {
  it("renders the superseded number struck through, with the time", () => {
    ...
    const line = container.querySelector(".zb-rv__superseded")!;
...
describe("absence", () => {
  ...
  it.each(ABSENT_REASONS)("%s renders a word and a sentence, never an em dash", (reason) => {
    ...
    const group = screen.getByRole("group");
    expect(group.getAttribute("data-zb-absent")).toBe(reason);
    expect(container.querySelector(".zb-rv__absent-word")).toHaveTextContent(ABSENCE[reason].short);
    expect(container.querySelector(".zb-rv__absent-detail")!.textContent!.length).toBeGreaterThan(
      10,
    );
...
describe("delta", () => {
  ...
  it("draws no arrow when nothing moved", () => {
    ...
    expect(container.querySelector(".zb-rv__delta")).toBeNull();
  });
  ...
  it("carries direction as a glyph and a word, not a hue", () => {
    ...
    expect(container.querySelector("[data-zb-direction='up']")).toBeTruthy();
...
describe("the accessible name", () => {
  ...
  it("hides every inner node from the accessibility tree", () => {
    ...
    for (const selector of [".zb-rv__line", ".zb-rv__qualifiers"]) {
