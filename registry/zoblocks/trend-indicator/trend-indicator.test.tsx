...
describe("minimum points", () => {
  ...
  it("draws no line for two points", () => {
    ...
    expect(container.querySelector(".zb-trend__chart")).toBeNull();
...
describe("significance", () => {
  ...
  it("marks the noise state so the colour goes neutral", () => {
    ...
    expect(screen.getByRole("figure").hasAttribute("data-zb-noise")).toBe(true);
  });
...
describe("valence", () => {
  ...
  it("carries direction as a glyph as well as a hue", () => {
    ...
    expect(container.querySelector("[data-zb-direction='falling']")).toBeTruthy();
    expect(container.querySelector(".zb-trend__delta")).toHaveTextContent("-9");
  });
});
...
describe("geometry", () => {
  ...
  it("drops the line below 40px and keeps the readout", () => {
    ...
    expect(container.querySelector(".zb-trend__chart")).toBeNull();
    expect(container.querySelector(".zb-trend__readout")).toBeTruthy();
  });
});
...
describe("re-rendering in a flowsheet", () => {
  ...
  it("skips the render when nothing it draws from has changed", () => {
    ...
    const svg = container.querySelector(".zb-trend__chart");
    ...
    expect(container.querySelector(".zb-trend__chart")).toBe(svg);
  });

  it("re-renders when the width changes", () => {
    const { rerender, container } = render(<Host series={phq9} width={64} />);
    rerender(<Host series={phq9} width={160} />);
    expect(container.querySelector(".zb-trend__chart")).toHaveAttribute("width", "160");
  });

  it("re-renders when the points array is a different array", () => {
    const { rerender, container } = render(<Host series={phq9} width={64} />);
    const before = container.querySelector(".zb-trend__line")?.getAttribute("d");
    ...
    expect(container.querySelector(".zb-trend__line")?.getAttribute("d")).not.toBe(before);
  });

  it("re-renders when the valence flips, because the colour is the answer", () => {
    const { rerender, container } = render(<Host series={phq9} width={64} />);
    expect(container.querySelector(".zb-trend")).toHaveAttribute("data-zb-judgement", "better");
    ...
    expect(container.querySelector(".zb-trend")).toHaveAttribute("data-zb-judgement", "worse");
  });
