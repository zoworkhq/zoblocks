...
describe("SafetyPlan", () => {
  ...
  it("defaults to patient density", () => {
    const view = render(<SafetyPlan steps={PLAN} />);
    expect(
      (view.container.querySelector(".zb-accordion") as HTMLElement).getAttribute(
        "data-zb-density",
      ),
    ).toBe("patient");
  });
  ...
  it("uses the heading level it is given", () => {
    ...
    expect(view.container.querySelectorAll("h2.zb-accordion__heading")).toHaveLength(6);
  });
  ...
  it("renders a contact who has only a name", () => {
    ...
    expect(crisis.querySelector(".zb-safety-plan__detail")).toBeNull();
    expect(crisis.querySelector(".zb-safety-plan__availability")).toBeNull();
  });
  ...
  it("omits the revision line entirely when no date is recorded", () => {
    ...
    expect(view.container.querySelector(".zb-safety-plan__revised")).toBeNull();
  });
});
