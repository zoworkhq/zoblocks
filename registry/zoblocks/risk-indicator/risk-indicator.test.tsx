...
describe("staleness", () => {
  ...
  it("says expired rather than old past the validity window", () => {
    ...
    expect(group.getAttribute("data-zb-freshness")).toBe("expired");
  ...
  it("treats a missing validity window as unbounded, not as fresh", () => {
    ...
    expect(screen.getByRole("group").getAttribute("data-zb-freshness")).toBe("unbounded");
  });
...
describe("attribution", () => {
  it("renders the top drivers with direction and weight", () => {
    ...
    const rows = container.querySelectorAll(".zb-risk__driver");
    expect(rows).toHaveLength(4);
    expect(rows[0]).toHaveTextContent("3 admissions / 6 mo");
    expect(rows[0]?.getAttribute("data-zb-direction")).toBe("raises");
    expect(rows[3]?.getAttribute("data-zb-direction")).toBe("lowers");
  });
  ...
  it("stays quiet when attribution is spread", () => {
    ...
    expect(container.querySelector(".zb-risk__concentration")).toBeNull();
  });
  ...
  it("renders unweighted drivers without inventing a bar", () => {
    ...
    expect(container.querySelector(".zb-risk__driver-bar")).toBeNull();
    expect(container.querySelector("[data-zb-unweighted]")).toHaveTextContent("contributing");
  });
...
describe("framing", () => {
  ...
  it("demotes the numeral and rounds it", () => {
    ...
    expect(container.querySelector(".zb-risk__probability")).toHaveTextContent("31");
    expect(container.querySelector(".zb-risk__probability")).not.toHaveTextContent("31.4");
  });
  ...
  it("leads with the band", () => {
    ...
    expect(container.querySelector(".zb-risk__head [data-zb-scale='risk']")).toBeTruthy();
  });
...
describe("bands", () => {
  ...
  it("renders unknown as its own state, not as low", () => {
    ...
    const group = screen.getByRole("group");
    expect(group.getAttribute("data-zb-band")).toBe("unknown");
  ...
  it("says nothing numeric about an unscored patient", () => {
    ...
    expect(container.querySelector(".zb-risk__probability")).toBeNull();
  });
