...
describe("Ant Design v6 parity, in behaviour", () => {
  it("resolves an unset mode to start, as antd 6.6.0's source does", () => {
    ...
    expect(container.querySelector("ol")?.className).toContain("zb-timeline--start");
  });
  ...
  it("treats v5's left and right as v6's start and end", () => {
    ...
    expect(container.querySelector("ol")?.className).toContain("zb-timeline--end");
  });
  ...
  it("maps the four preset colours to semantic tokens, not palette values", () => {
    // A component reaching past --zb-status-critical to a hex would ignore
    ...
    expect(container.querySelector<HTMLElement>(".zb-timeline__node")?.style.color).toContain(
      "--zb-status-critical",
    );
  });
  ...
  it("passes an unrecognised colour through untouched", () => {
    ...
    expect(container.querySelector<HTMLElement>(".zb-timeline__node")?.style.color).toBe(
      "rgb(18, 52, 86)",
    );
  });
  ...
  it("exposes antd's semantic slots for classNames and styles", () => {
    ...
    expect(container.querySelector<HTMLElement>(".zb-timeline__node")?.style.opacity).toBe("0.5");
  });
  ...
  it("rebuilds the whole class table when a caller supplies prefixCls", () => {
    ...
    expect(container.querySelector(".zb-timeline__item")).toBeNull();
  });
  ...
  it("keeps an item's own className and per-slot overrides", () => {
    ...
    expect(container.querySelector<HTMLElement>(".zb-timeline__content")?.style.color).toBe("red");
  });
  ...
  it("reads both halves of v5's position vocabulary", () => {
    ...
    expect(items[0]?.className).toContain("zb-timeline__item--start");
    expect(items[1]?.className).toContain("zb-timeline__item--end");
    expect(items[2]?.className).toContain("zb-timeline__item--end");
  });
  ...
  it("prefers v6's placement over v5's position when both are given", () => {
    ...
    expect(container.querySelector("li")?.className).toContain("zb-timeline__item--start");
  });
