...
const beatOf = (container: HTMLElement) =>
  root(container)?.style.getPropertyValue("--zb-loader-beat");
...
describe("PulseLoader", () => {
  ...
  it("renders the rhythm line three times: a track, a tail, and a head", () => {
    ...
    expect(lines[0]).toHaveClass("zb-loader__track");
    expect(lines[1]).toHaveClass("zb-loader__tail");
    expect(lines[2]).toHaveClass("zb-loader__head");
  });
  ...
  it("draws the heart once rather than once per loop", () => {
    ...
    const view = render(<PulseLoader label="Loading" />);
    const drawn = view.container.querySelectorAll(".zb-loader__draw");
  ...
  it("normalises both heart paths so one draw duration covers both", () => {
    const view = render(<PulseLoader label="Loading" />);
    for (const path of view.container.querySelectorAll(".zb-loader__draw")) {
  ...
  it("keeps the heart at the threshold size", () => {
    ...
    expect(root(view.container)).toHaveAttribute("data-zb-loader", "pulse");
  });
  ...
  it("thickens the stroke floor at small sizes so the line survives", () => {
    ...
    expect(root(small.container)?.style.getPropertyValue("--zb-loader-stroke")).toBe("2px");

    const large = render(<PulseLoader label="Loading" size={88} />);
    expect(root(large.container)?.style.getPropertyValue("--zb-loader-stroke")).toBe("2.4px");
  });
  ...
  describe("PageLoader", () => {
    it("covers the page and shows its label without being asked", () => {
      ...
      expect(element).toHaveClass("zb-loader--page");
      expect(view.container.querySelector(".zb-loader__label")?.textContent).toBe(
    ...
    it("is still a pulse at the page size", () => {
      ...
      expect(root(view.container)).toHaveAttribute("data-zb-loader", "pulse");
      expect(root(view.container)?.style.getPropertyValue("--zb-loader-size")).toBe("88px");
    });

    it("can be overridden like any other loader", () => {
      const view = render(<PageLoader label="Loading" mode="overlay" size="lg" />);
      expect(root(view.container)).toHaveClass("zb-loader--overlay");
