...
describe("encounter context", () => {
  ...
  it("renders a select the host can change, and announces the reason when none is chosen", () => {
    ...
    expect(select).toHaveAttribute("data-zb-encounter", "none");
...
describe("collapse", () => {
  ...
  it("keeps the detail in the DOM behind a disclosure", () => {
    ...
    const detail = container.querySelector(".zb-chart-header__detail");
    ...
    expect(container.querySelector(".zb-chart-header__detail")).toHaveAttribute("hidden");
  });
  ...
  it("draws the same strip at both heights", () => {
    ...
    const expanded = container.querySelector(".zb-chart-header__strip")?.textContent;

    rerender(<ChartHeader patient={patient} identifiers={identifiers} safety={safety} collapsed />);
    const collapsed = container.querySelector(".zb-chart-header__strip")?.textContent;
...
describe("the safety strip", () => {
  ...
  it("shows an expired hold as expired rather than removing it", () => {
    ...
    const fact = screen.getByText("Involuntary hold").closest(".zb-chart-header__fact");
    expect(fact).toHaveAttribute("data-zb-expired", "");
    expect(fact).toHaveAttribute("data-zb-tone", "critical");
...
describe("the header itself", () => {
  ...
  it("shows the selected encounter in the select when the host can change it", async () => {
    ...
    expect(select).toHaveAttribute("data-zb-encounter", "selected");
  ...
  it("carries the surface on the bar, so a stylesheet can react without a class", () => {
    ...
    expect(screen.getByRole("banner")).toHaveAttribute("data-zb-surface", "results");
  });
});
