...
describe("rendering", () => {
  it.each(EVERY_STEP)("%s/%s renders a glyph and a word", (scale, id) => {
    ...
    const chip = container.querySelector("[data-zb-status]")!;
    const glyph = chip.querySelector(".zb-cs__glyph")!;

    expect(glyph, "no glyph element").toBeTruthy();
    expect(glyph.getAttribute("data-zb-glyph")).toBe(resolveStatus(scale, id).glyph);
  ...
  it("switches register without switching state", () => {
    ...
    expect(chip.getAttribute("data-zb-step")).toBe("entered-in-error");
  ...
  it("renders both the full word and the abbreviation, and lets CSS pick", () => {
    ...
    expect(container.querySelector("[data-zb-full]")).toHaveTextContent("Not assessed");
    expect(container.querySelector("[data-zb-abbr]")).toHaveTextContent("N/A");
  ...
  it("carries the tone as data rather than as an inline colour", () => {
    ...
    const chip = container.querySelector("[data-zb-status]") as HTMLElement;
...
describe("interaction", () => {
  ...
  it("keeps a 24px target at both densities", () => {
    ...
    for (const density of ["default", "compact"] as const) {
      ...
      expect(chip.getAttribute("data-zb-density")).toBe(density);
      unmount();
    }
  });
});
