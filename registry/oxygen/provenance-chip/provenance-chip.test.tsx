...
describe("source classes", () => {
  ...
  it.each(SOURCES)("%s renders its own glyph", (source) => {
    ...
    expect(container.querySelector(`[data-ox-glyph='${SOURCE_GLYPH[source]}']`)).toBeTruthy();
    expect(container.querySelector("[data-ox-provenance]")?.getAttribute("data-ox-source")).toBe(
      source,
    );
  });
  ...
  it("does not treat patient-report as a defect", () => {
    ...
    const chip = container.querySelector("[data-ox-provenance]")!;
    expect(chip.hasAttribute("data-ox-unconfirmed")).toBe(false);
...
describe("staleness", () => {
  ...
  it("shows the age without a policy, but makes no judgement", () => {
    ...
    const age = container.querySelector(".ox-prov__age")!;
    expect(age).toHaveTextContent("12 min");
    expect(age.getAttribute("data-ox-staleness")).toBe("unknown");
  });
  ...
  it("puts the age on the chip once there is a policy", () => {
    ...
    const age = container.querySelector(".ox-prov__age")!;
    expect(age.getAttribute("data-ox-staleness")).toBe("stale");
  ...
  it("renders no age at all without `now`", () => {
    ...
    expect(container.querySelector(".ox-prov__age")).toBeNull();
  });
...
describe("the ledger", () => {
  ...
  it("reads by resource id", () => {
    ...
    expect(screen.getByRole("img").getAttribute("data-ox-source")).toBe("clinic");
  });
  ...
  it("prefers the versioned entry", () => {
    ...
    expect(screen.getByRole("img").getAttribute("data-ox-source")).toBe("amended");
  });
