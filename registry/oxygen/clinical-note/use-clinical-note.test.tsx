...
import { useClinicalNote } from "@/lib/oxygen-clinical-note";
...
describe("F2", () => {
  it("moves to the next blank from the keyboard", async () => {
    ...
    const editor = container.querySelector<HTMLElement>(".ox-note-doc");
...
describe("the editable region", () => {
  it("carries the accessible name the host supplied", () => {
    ...
    const { container } = render(<Harness value={withAi()} />);
    expect(container.querySelector(".ox-note-doc")).toHaveAttribute("aria-label", "Note body");
  });
...
describe("the origins toggle", () => {
  it("is off until asked, and redraws when flipped", async () => {
    ...
    expect(container.querySelectorAll(".ox-note-pv")).toHaveLength(0);
