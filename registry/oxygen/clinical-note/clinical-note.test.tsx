...
import { NODES, expand } from "@oxygenui-design/clinical-note-core";
import { composition, unreviewedAi } from "@oxygenui-design/clinical-note-core";
...
describe("the origins ribbon", () => {
  ...
  it("tints the document when turned on", async () => {
    ...
    await waitFor(() => {
      expect(container.querySelectorAll(".ox-note-pv").length).toBeGreaterThan(0);
    });
  });
  ...
  it("gives each origin a class of its own, so colour is never the only channel", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /where each passage came from/i }));
    await waitFor(() => {
      expect(container.querySelector(".ox-note-pv-copied")).toBeTruthy();
      expect(container.querySelector(".ox-note-pv-ai")).toBeTruthy();
    });
  });
});
...
describe("toolbar commands", () => {
  it("applies bold and italic to the document", async () => {
    ...
    expect(container.querySelector(".ox-note-doc")).toBeInTheDocument();
  });
