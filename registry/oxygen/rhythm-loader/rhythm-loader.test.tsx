...
describe("RhythmLoader", () => {
  it("draws one complex on a baseline, three times over", () => {
    ...
    expect(paths[0]).toHaveClass("ox-loader__track");
    expect(paths[1]).toHaveClass("ox-loader__tail");
    expect(paths[2]).toHaveClass("ox-loader__head");
  });
  ...
  it("never scales anything — the whole point of this loader", () => {
    ...
    expect(view.container.querySelector(".ox-loader__core")).toBeNull();
  });

  it("stays legible at the smallest inline size", () => {
    const view = render(<RhythmLoader label="Loading" size="sm" />);
    expect(root(view.container)?.style.getPropertyValue("--ox-loader-size")).toBe("20px");
    expect(root(view.container)?.style.getPropertyValue("--ox-loader-stroke")).toBe("2px");
  ...
  it.each([
    ...
    expect(root(view.container)?.style.getPropertyValue("--ox-loader-beat")).toBe(expected);
  });

  it("clamps bpm to the resting range like every cardiac loader", () => {
    const fast = render(<RhythmLoader label="Loading" bpm={220} />);
    expect(root(fast.container)?.style.getPropertyValue("--ox-loader-beat")).toBe("600ms");
  });
});
