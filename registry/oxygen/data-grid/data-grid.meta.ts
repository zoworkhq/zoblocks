...
export default defineComponentMeta({
  ...
  a11y: [
    ...
    },
    {
      ...
      detail:
        'A reader on row 4 of 24 in a cohort of 1,438 is told exactly that. Where the source will not give a total the attribute is -1, which is ARIA\'s "not known", rather than the page size dressed up as an answer.',
    },
    {
...
/>`,
  ...
  domain: {
    ...
    clinicalContext:
      "A worklist is the screen clinical work is dispatched from, so its two failure modes are structural rather than cosmetic: it can imply completeness it does not have, and it can move a row out from under an action. Coverage answers the first and held arrivals answer the second.",
  ...
  fixtures: ["wardPotassium", "unknownTotal"],
...
gridUnseenCount({ shown: 24, total: "unknown" }); // → null`,
    },
    {
      ...
      description:
        "The load-bearing line in the engine. A potassium that has not come back is not a low potassium — sorting ascending and finding four “Awaiting” rows above the 3.2 tells the reader the sickest patient is fine.",
      fixture: "wardPotassium",
