...
const slug = (container: HTMLElement) => container.querySelector(".ox-loader__slug");
...
describe("InfusionLoader", () => {
  ...
  describe("indeterminate", () => {
    it("stays a status region and claims no value", () => {
      ...
      expect(element).toHaveAttribute("data-ox-determinate", "false");
    });
  ...
  describe("determinate", () => {
    it("becomes a progressbar with a full ARIA value set", () => {
      ...
      expect(element).toHaveAttribute("data-ox-determinate", "true");
    });
    ...
    it("takes its accessible name from the label a sighted reader sees", () => {
      ...
      expect(label).toHaveClass("ox-loader__label");
    });
  ...
  it("draws a capsule track and exactly one slug", () => {
    ...
    expect(rects[0]).toHaveClass("ox-loader__track");
    expect(rects[1]).toHaveClass("ox-loader__slug");
  });

  it("keeps the slug inside the capsule at full", () => {
    const view = render(<InfusionLoader label="Importing" progress={100} />);
    const track = view.container.querySelector(".ox-loader__track");
