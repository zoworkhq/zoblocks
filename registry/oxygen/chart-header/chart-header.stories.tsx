...
export const Collapsed: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const detail = canvasElement.querySelector(".ox-chart-header__detail");
    ...
    expect(canvasElement.querySelector(".ox-chart-header__strip")?.textContent).toContain(
      "Penicillin",
    );
    expect(
...
export const ThreeEncountersOpen: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    expect(select.getAttribute("data-ox-encounter")).toBe("none");
...
export const AllergiesNotAsked: Story = {
  ...
  play: async ({ canvasElement }) => {
    const fact = canvasElement.querySelector('[data-ox-kind="allergy"]');
    expect(fact?.getAttribute("data-ox-tone")).toBe("absent");
...
export const CodeStatusNotRecorded: Story = {
  ...
  play: async ({ canvasElement }) => {
    const fact = canvasElement.querySelector('[data-ox-kind="code-status"]');
    expect(fact?.getAttribute("data-ox-tone")).toBe("absent");
  },
};
...
export const IsolationAndFallRisk: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const kinds = [...canvasElement.querySelectorAll("[data-ox-kind]")].map((el) =>
      el.getAttribute("data-ox-kind"),
    );
    expect(kinds).toEqual(["allergy", "code-status", "isolation", "fall-risk"]);
    expect(
      canvasElement.querySelector('[data-ox-kind="isolation"]')?.getAttribute("data-ox-tone"),
    ).toBe("critical");
  },
};
...
export const InvoluntaryHold: Story = {
  ...
  play: async ({ canvasElement }) => {
    const fact = canvasElement.querySelector('[data-ox-kind="legal-status"]');
    expect(fact?.hasAttribute("data-ox-expired")).toBe(false);
...
export const HoldExpired: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // signal as its lapse.
    expect(fact?.hasAttribute("data-ox-expired")).toBe(true);
