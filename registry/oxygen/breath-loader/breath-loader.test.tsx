...
describe("BreathLoader", () => {
  it("renders three rings and one core", () => {
    ...
    expect(view.container.querySelectorAll(".ox-loader__ring")).toHaveLength(3);
    expect(view.container.querySelectorAll(".ox-loader__core")).toHaveLength(1);
  });
  ...
  it("keeps the rings as siblings so the CSS stagger applies", () => {
    ...
    const view = render(<BreathLoader label="Loading" />);
    const rings = [...view.container.querySelectorAll(".ox-loader__ring")];
  ...
  it("breathes at roughly fifteen a minute by default", () => {
    const view = render(<BreathLoader label="Loading" />);
    expect(root(view.container)?.style.getPropertyValue("--ox-loader-cycle")).toBe("4000ms");
  });
  ...
  it.each([
    ...
    expect(root(view.container)?.style.getPropertyValue("--ox-loader-cycle")).toBe(expected);
  });
  ...
  it("clamps speed so nothing can be driven into a flicker", () => {
    ...
    expect(root(fast.container)?.style.getPropertyValue("--ox-loader-cycle")).toBe("2000ms");

    const slow = render(<BreathLoader label="Loading" speed={0.01} />);
    expect(root(slow.container)?.style.getPropertyValue("--ox-loader-cycle")).toBe("8000ms");
  });

  it("has no beat, because it is not a cardiac loader", () => {
    const view = render(<BreathLoader label="Loading" />);
    expect(view.container.querySelector(".ox-loader__beat")).toBeNull();
    expect(view.container.querySelector(".ox-loader__head")).toBeNull();
  });
});
