/**
 * The console client, and what it makes of a refusal.
 *
...
import { consoleApi, readOrigin, readToken, type Fetcher } from "../src/console";

const credential = { origin: "https://console.example.test", token: "oxy_live_" + "x".repeat(30) };
...
describe("what it sends", () => {
  it("carries the key on every call and never in the URL", async () => {
    ...
    await consoleApi(fetcher, credential).themes();

    expect(seen[0]!.url).toBe("https://console.example.test/api/v1/themes");
  ...
  it("escapes a slug rather than pasting it into a path", async () => {
    ...
    await consoleApi(fetcher, credential).resolved("../../admin");
  ...
  it("asks for a version only when there is one", async () => {
    ...
    const api = consoleApi(fetcher, credential);
  ...
  it("sends one colour to propose, and calls POST", async () => {
    ...
    await consoleApi(fetcher, credential).propose("clinical", "#1d63c9");
...
describe("what it makes of a refusal", () => {
  it("passes the console's own words through when there are any", async () => {
    const { fetcher } = stub([
      { status: 422, body: { error: "1 problem(s) with that ramp.", detail: ["2.98:1"] } },
    ]);
    const result = await consoleApi(fetcher, credential).propose("clinical", "#0f766e");
  ...
  it("says what to do about a 401 rather than repeating the number", async () => {
    ...
    const result = await consoleApi(fetcher, credential).themes();
  ...
  it("does not promise a 404 means the theme is gone", async () => {
    ...
    const result = await consoleApi(fetcher, credential).resolved("clinical");
  ...
  it("names the address when the network is the problem", async () => {
    ...
    // and a console origin that is not the allowed one. Both need the address.
    if (!result.ok) expect(result.error).toContain("https://console.example.test");
  });
...
describe("what a designer may type", () => {
  it("accepts an origin and drops what is not one", () => {
    expect(readOrigin("https://console.oxygenui.design")).toBe("https://console.oxygenui.design");
    expect(readOrigin("  https://console.oxygenui.design/  ")).toBe(
      "https://console.oxygenui.design",
    );
  ...
  it("refuses a path, so a pasted deep link is a correction not a 404", () => {
    expect(readOrigin("https://console.oxygenui.design/themes/clinical")).toBeUndefined();
  });

  it("refuses plaintext http anywhere but localhost", () => {
    // A bearer token over http is the token given away.
    expect(readOrigin("http://console.oxygenui.design")).toBeUndefined();
  });

  it("refuses nonsense", () => {
    for (const bad of ["", "   ", "console.oxygenui.design", "javascript:alert(1)"]) {
