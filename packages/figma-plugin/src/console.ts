/**
 * The console's API, as plain data in and plain data out.
 *
 ...
 * console is deploying. Those are three different sentences for a designer and
...
export interface Credential {
  /** Origin only — `https://console.oxygenui.design`. No path. */
  origin: string;
  token: string;
}
 ...
 *
 * The status codes are the console's own vocabulary and each means something
...
async function readError(response: Response): Promise<Outcome<never>> {
  ...
  const error =
    typeof body.error === "string"
      ? body.error
      : response.status === 401
        ? "This plugin is not connected. Paste a Figma key from the console."
        : response.status === 404
...
async function call<T>(
  ...
  try {
     ...
     * produce this are an offline laptop and a console origin that is not the
  ...
  try {
    ...
    return { ok: false, error: "The console sent something that is not JSON." };
  }
}

export function consoleApi(fetcher: Fetcher, credential: Credential) {
  return {
    themes: () =>
