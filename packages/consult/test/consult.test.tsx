 ...
 *
 * consult-core proves the engine and consult-react proves the behaviour, so
...
let ids = 0;
const setup = (over: Partial<ConsultProps> = {}) => {
  const props: ConsultProps = {
  ...
  return { user: userEvent.setup(), ...render(<Consult {...props} />) };
};
...
describe("crisis", () => {
  ...
  it("disables the field so the conversation cannot continue", async () => {
    ...
    expect(screen.getByRole("combobox", { name: DEFAULT_LOCALE.dockLabel })).toBeDisabled();
  });
...
describe("modes and shortcuts", () => {
  ...
  it("keeps focus in the field while arrowing, per the combobox pattern", async () => {
    ...
    expect(field).toHaveAttribute("aria-activedescendant", "consult-shortcuts-option-1");
  });
