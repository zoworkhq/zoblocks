...
/**
 * zoblocks-codemod — run a migration over files you name.
 *
...
if (!name || !TRANSFORMS[name] || files.length === 0) {
  stdout.write(
    [
      "Usage: zoblocks-codemod <transform> <file...> [--write]",
      "",
      "Transforms:",
...
for (const file of files) {
  ...
  for (const note of notes) {
    ...
    stdout.write(
      `  ${note.severity === "action" ? "TODO" : "note"}  ${file}:${note.line}  ${note.message}\n`,
    );
  }
}
