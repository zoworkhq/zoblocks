 ...
 * element's promise is that `<zb-pulse-loader>` works the moment the script
 ...
 * `styles/zoblocks-loader.css`, because a component copied into a customer's repo
...
:host {
  --_size: var(--zb-loader-size, 56px);
  --_color: var(--zb-loader-color, var(--zb-accent, currentColor));
  --_track: var(--zb-loader-track, color-mix(in oklab, var(--_color) 22%, transparent));
  --_stroke: var(--zb-loader-stroke, 2.4px);
  --_beat: var(--zb-loader-beat, 1000ms);
  --_cycle: var(--zb-loader-cycle, 4000ms);
  --_scrim: var(--zb-loader-scrim, color-mix(in oklab, var(--zb-bg, Canvas) 72%, transparent));
  ...
  gap: 0.6em;
  color: var(--_color);
  font-family: var(--zb-font-sans, inherit);
  font-size: var(--zb-text-sm, 0.875rem);
  line-height: 1.45;
  text-align: center;
  animation: zb-loader-enter 160ms var(--zb-ease, ease-out) both;
}

:host([hidden]) { display: none; }

@keyframes zb-loader-enter { from { opacity: 0; } to { opacity: 1; } }
...
.label { color: var(--zb-text, currentColor); font-weight: 500; }
.value { color: var(--zb-text, currentColor); font-variant-numeric: tabular-nums; font-weight: 600; }
.hint { color: var(--zb-text-muted, currentColor); font-size: var(--zb-text-xs, 0.8125rem); max-width: 34ch; }
...
.draw {
  ...
  animation: zb-loader-draw 700ms var(--_ease-out) forwards;
}
.beat {
  transform-box: fill-box;
  transform-origin: center;
  animation: zb-loader-beat var(--_beat) var(--_ease) infinite;
  animation-delay: 400ms;
}
.head {
  stroke-dasharray: 22 200;
  stroke-dashoffset: 22;
  animation: zb-loader-head var(--_beat) cubic-bezier(0.4, 0, 0.6, 1) infinite;
  animation-delay: 400ms;
}
.tail {
  ...
  opacity: 0.35;
  animation: zb-loader-tail var(--_beat) cubic-bezier(0.4, 0, 0.6, 1) infinite;
  animation-delay: 400ms;
}
@keyframes zb-loader-draw { to { stroke-dashoffset: 0; } }
@keyframes zb-loader-beat { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.07); } }
@keyframes zb-loader-head { to { stroke-dashoffset: -100; } }
@keyframes zb-loader-tail { to { stroke-dashoffset: -78; } }
...
.ring {
  ...
  transform-origin: center;
  opacity: 0;
  animation: zb-loader-ring var(--_cycle) cubic-bezier(0.2, 0.6, 0.4, 1) infinite;
}
...
.core {
  transform-box: fill-box;
  transform-origin: center;
  animation: zb-loader-core var(--_cycle) var(--_ease) infinite;
}
@keyframes zb-loader-ring {
  0% { transform: scale(0.3); opacity: 0.55; }
  70% { opacity: 0.08; }
  100% { transform: scale(1); opacity: 0; }
}
@keyframes zb-loader-core { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.18); } }

/* Helix ----------------------------------------------------------- */
.dot {
  transform-box: fill-box;
  transform-origin: center;
  animation: zb-loader-helix calc(var(--_cycle) * 0.65) ease-in-out infinite;
  animation-delay: calc(var(--_cycle) * 0.65 * var(--zb-loader-phase, 0));
}
@keyframes zb-loader-helix {
...
/* Infusion -------------------------------------------------------- */
.slug { animation: zb-loader-slug calc(var(--_cycle) * 0.7) var(--_ease) infinite alternate; }
:host([progress]) .slug { animation: none; transform: none; }
@keyframes zb-loader-slug { from { transform: translateX(0); } to { transform: translateX(88px); } }

@keyframes zb-loader-still { 0%, 100% { opacity: 1; } 50% { opacity: 0.55; } }
...
@media (prefers-reduced-motion: reduce) {
  ...
  :host(:not([motion="full"])) .art { animation: zb-loader-still 2400ms var(--_ease) infinite; }
}
...
:host([motion="reduced"]) .art { animation: zb-loader-still 2400ms var(--_ease) infinite; }
