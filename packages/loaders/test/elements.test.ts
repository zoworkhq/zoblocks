...
describe("registration", () => {
  ...
  it("tolerates a second import of the same module", async () => {
    ...
    expect(customElements.get("zb-pulse-loader")).toBeTypeOf("function");
  });
...
describe("attribute reactivity", () => {
  it("re-renders when an attribute changes", () => {
    const element = mount("zb-breath-loader", { label: "First" });
  ...
  it.each([
    ...
    const element = mount("zb-breath-loader", { size });
    expect(element.style.getPropertyValue("--zb-loader-size")).toBe(expected);
  });

  it("clamps an absurd size rather than rendering it", () => {
    const element = mount("zb-breath-loader", { size: "9000" });
    expect(element.style.getPropertyValue("--zb-loader-size")).toBe("480px");
  });
  ...
  it.each([
    ...
    const element = mount("zb-pulse-loader", { bpm });
    expect(element.style.getPropertyValue("--zb-loader-beat")).toBe(expected);
  });

  it("clamps speed so nothing can be driven into a flicker", () => {
    const fast = mount("zb-breath-loader", { speed: "40" });
    expect(fast.style.getPropertyValue("--zb-loader-cycle")).toBe("2000ms");

    const slow = mount("zb-breath-loader", { speed: "0.01" });
    expect(slow.style.getPropertyValue("--zb-loader-cycle")).toBe("8000ms");
  });

  it.each(["auto", "reduced", "full"])(
    "keeps motion=%s on the host for CSS to act on",
    (motion) => {
      const element = mount("zb-breath-loader", { motion });
...
describe("pulse", () => {
  it("draws the heart open at both sides", () => {
    const element = mount("zb-pulse-loader");
  ...
  it(`renders the rhythm line alone below ${PULSE_MIN_SIZE_PX}px`, () => {
    const element = mount("zb-pulse-loader", { size: "24" });
    const html = element.shadowRoot?.innerHTML ?? "";
    expect(html).not.toContain(LOADER_ART.heartTop);
    expect(html).toContain(LOADER_ART.strip);
    expect(element.getAttribute("data-zb-loader")).toBe("rhythm");
  });

  it("keeps the heart at the threshold size", () => {
    const element = mount("zb-pulse-loader", { size: String(PULSE_MIN_SIZE_PX) });
    expect(element.getAttribute("data-zb-loader")).toBe("pulse");
  });
});

describe("helix", () => {
  it("renders two strands of nine with distinct phases", () => {
    const element = mount("zb-helix-loader");
    const dots = [...(element.shadowRoot?.querySelectorAll(".dot") ?? [])];
    expect(dots).toHaveLength(18);

    const phases = dots.map((dot) =>
      (dot as SVGElement).style.getPropertyValue("--zb-loader-phase"),
    );
  ...
  it("offsets the second strand by half a turn", () => {
    ...
    const phases = [...(element.shadowRoot?.querySelectorAll(".dot") ?? [])].map((dot) =>
      Number((dot as SVGElement).style.getPropertyValue("--zb-loader-phase")),
    );
...
describe("infusion", () => {
  it("is a status region with a drifting slug when indeterminate", () => {
    const element = mount("zb-infusion-loader", { label: "Preparing the export" });
    expect(element.getAttribute("role")).toBe("status");
  ...
  it("becomes a progressbar with a full value set when determinate", () => {
    const element = mount("zb-infusion-loader", { label: "Importing records", progress: "42" });
  ...
  it("returns to a status region when progress is removed", () => {
    const element = mount("zb-infusion-loader", { progress: "42" });
  ...
  it.each([
    ...
    const element = mount("zb-infusion-loader", { progress });
...
describe("timing and events", () => {
  ...
  it("stays hidden until the delay elapses", () => {
    const element = mount("zb-breath-loader", { delay: "200" });
  ...
  it("emits show, slow, and hide across the shadow boundary", () => {
    const events: string[] = [];
    for (const type of ["zb-loader-show", "zb-loader-slow", "zb-loader-hide"]) {
    ...
    expect(events).toEqual(["zb-loader-show", "zb-loader-slow", "zb-loader-hide"]);
  });

  it("shows the stall wording, and says what is still possible", () => {
    const element = mount("zb-breath-loader", { "slow-after": "1000", "show-label": "true" });
    vi.advanceTimersByTime(1000);
  ...
  it("never claims a stall when slow-after is disabled", () => {
    const element = mount("zb-breath-loader", { "slow-after": "0", "show-label": "true" });
  ...
  it("clears its timers when removed from the document", () => {
    ...
    const element = mount("zb-breath-loader", { "slow-after": "1000" });
    element.remove();
