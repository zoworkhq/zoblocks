...
describe("ProposalCard", () => {
  const api = (over: Partial<CopilotApi> = {}) =>
    ({
      proposal: {
        id: "p1",
        kind: "note-text",
...
describe("non-happy states", () => {
  ...
  it("sends structured feedback from both controls", async () => {
    ...
    // act on, which is the failure mode of every thumbs widget ever shipped.
  ...
  it("wires the crisis escalation hooks through from props", async () => {
    ...
    const { user } = renderCopilot({ onRiskProtocol });
