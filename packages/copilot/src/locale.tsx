...
export interface CopilotLocale {
  /** The dock */
  dockLabel: string;
  placeholder: string;
  ...
  reasoning: string;
  thinking: string;
  whyNotHelpful: string;
  feedbackReason: (reason: string) => string;
...
export const DEFAULT_LOCALE: CopilotLocale = {
  ...
  collapseDock: "Put the assistant away",
  // Not "Open assistant". A clinician who collapsed it knows what it is; what
  // they need to know is that this is the same one, still holding their thread.
  reopen: "Reopen the assistant",
  ...
  reasoning: "Reasoning",
  thinking: "Working…",
  whyNotHelpful: "What was wrong with it?",
...
const LocaleContext = createContext<CopilotLocale>(DEFAULT_LOCALE);

export function CopilotLocaleProvider(props: {
  value?: Partial<CopilotLocale>;
  children: ReactNode;
}): ReactNode {
...
export function useLocale(): CopilotLocale {
