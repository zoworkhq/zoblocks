 ...
 *
 * `crisis` sits second rather than first for one reason: an out-of-scope
 * question should be redirected before anything scans it for risk language, so
 * that "write me a poem" gets a scope refusal rather than an escalation. Every
 * other ordering is safety-first.
 *
...
import { assembleRequest, resolveContext, type ConsultContextResolver } from "./context.js";
import { auditExchange, type AuditSink } from "./audit.js";
import { decideTool, type ToolCall } from "./actions.js";
import { consultError, PhiNotPermittedError } from "./errors.js";
import { runChecks, type CheckResult } from "./checks.js";
import type { ConsultMode } from "./modes.js";
import type { ConsultProvider, ConsultEvent } from "./provider.js";
...
export interface PipelineDeps {
  ...
  readonly contextResolver?: ConsultContextResolver;
...
export async function runExchange(options: RunExchangeOptions): Promise<ExchangeOutcome> {
  ...
  if (!scope.inScope) {
    const error = consultError("out-of-scope", scopeMessage(scope.reason), {
      retryable: false,
  ...
  try {
    ...
    if (cause instanceof PhiNotPermittedError) {
      dispatch({
        type: "fail",
        error: consultError("phi-not-permitted", cause.message, { retryable: false, cause }),
      });
      throw cause;
    }
    dispatch({
      type: "fail",
      error: consultError("provider", "The request could not be assembled.", { cause }),
    });
    return { kind: "failed" };
  }
  ...
  try {
    ...
    dispatch({
      type: "fail",
      error: consultError("network", "The connection to the assistant failed.", { cause }),
    });
    await emitAudit(deps, {
      exchangeId,
      mode,
 ...
 */
function applyLocally(state: SessionState, event: ConsultEvent): SessionState {
