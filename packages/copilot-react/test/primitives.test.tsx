...
describe("CopilotLiveRegion", () => {
  it("is a polite status region, not an assertive alert", () => {
    ...
    render(<CopilotLiveRegion message="Answer complete." />);
  ...
  it("is visually hidden but present in the accessibility tree", () => {
    render(<CopilotLiveRegion message="Answering." />);
    const region = screen.getByRole("status");
...
describe("CopilotAnswerRegion", () => {
  it("is always aria-live off, so tokens are never announced", () => {
    const { container } = render(
      <CopilotAnswerRegion busy>Rate control is reasonable.</CopilotAnswerRegion>,
    );
  ...
  it("clears busy when the stream finishes", () => {
    const { container } = render(<CopilotAnswerRegion busy={false}>Done.</CopilotAnswerRegion>);
...
describe("CopilotDockRegion", () => {
  it("is a named complementary landmark, not a bare floating div", () => {
    render(<CopilotDockRegion>dock</CopilotDockRegion>);
  ...
  it("accepts a custom label for localisation", () => {
    render(<CopilotDockRegion label="Assistant clinique">dock</CopilotDockRegion>);
    expect(screen.getByRole("complementary", { name: "Assistant clinique" })).toBeInTheDocument();
  });
});

describe("CopilotRoot", () => {
  const api = (over: Partial<CopilotApi> = {}) => ({ suppressed: false, ...over }) as CopilotApi;

  it("renders nothing at all when suppressed — Law 5", () => {
    const { container } = render(
      <CopilotRoot api={api({ suppressed: true })}>
        <div>dock</div>
      </CopilotRoot>,
    );
  ...
  it("throws a useful error when the context is used outside the root", () => {
    ...
    expect(() => render(<Consumer />)).toThrow(/inside <CopilotRoot>/);
    spy.mockRestore();
  });

  it("provides the api to descendants", () => {
    const Consumer = () => <span>{useCopilotContext().suppressed ? "off" : "on"}</span>;
    render(
      <CopilotRoot api={api()}>
        <Consumer />
      </CopilotRoot>,
...
describe("accessibility", () => {
  it("has no axe violations across the primitives together", async () => {
    const { container } = render(
      <CopilotDockRegion>
        <CopilotLiveRegion message="Answer complete. 12 words, 3 sources." />
        <CopilotAnswerRegion busy={false}>
