...
import { createStaticProvider, type CopilotEvent, type Source } from "../src/provider.js";
...
describe("stage order", () => {
  it("still refuses an off-topic question — crisis running first costs nothing here", async () => {
    // The original order put scope first to stop this being escalated. The
    // crisis classifier never fires on it, so the protection was imaginary and
    // the ordering only cost real escalations elsewhere.
    const h = harness();
  ...
  it("escalates a patient at risk even when the mode would have refused it", async () => {
    // The case that forced the reorder. A clinician says their patient is in
    // danger while a reference-only mode happens to be selected; scope would
    // have redirected them to a different chip. Which chip is lit must not
    // decide whether someone in danger gets the escalation path.
    const h = harness();
    ...
    if (outcome.kind === "crisis") {
      expect(outcome.safety.crisis.severity).toBe("clinical-risk");
      expect(outcome.safety.crisis.audience).toBe("third-party");
    }
  });
