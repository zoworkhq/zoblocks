...
import { defineMode, type CopilotMode } from "./modes.js";
 ...
 * a score summary.
 */
export const betweenVisits: CopilotMode = defineMode({
  id: "between-visits",
 ...
 */
export const formulate: CopilotMode = defineMode({
  id: "formulate",
  label: "Formulate",
...
export const behavioralModes: readonly CopilotMode[] = [betweenVisits];
  ...
  constructor(modeId: string) {
    super(
      `Mode "${modeId}" is clinician-facing and cannot run on a patient-facing surface. ` +
        `Copilot has no patient-facing behavioral health configuration by design: ` +
 ...
 */
export function assertClinicianFacing(mode: CopilotMode, surface: "clinician" | "patient"): void {
