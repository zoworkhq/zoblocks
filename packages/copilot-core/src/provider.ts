 ...
 *
 * One rule shapes this whole file: **Copilot never talks to a model vendor.**
 * A `CopilotProvider` targets an endpoint the customer operates. That endpoint
...
export type CopilotEvent =
  ...
  | { readonly type: "error"; readonly error: CopilotError }
  | {
      readonly type: "done";
...
export interface CopilotProvider {
  ...
  send(request: CopilotRequest, signal: AbortSignal): AsyncIterable<CopilotEvent>;
}
