...
/**
 * The default host — Oxygen's own chrome.
 *
 * These are **reference primitives, not registry components.** Nothing here
 * ships through `oxygen add`, nothing here appears in the catalog, and no
 ...
 *
 * That distinction matters more than it looks. ADR 0010 says Oxygen's
 ...
 *
 * Structure only. Every colour, radius and duration comes from `--ox-*`, so
 * these restyle under a bridge exactly as a clinical component does — which
 * is the whole reason the Oxygen state is worth having in the switcher.
 * Styles are in `host.css`, imported by the consumer.
 *
 * Kept apart from `OxygenHost` so this module imports nothing but the
 * contract. `context.tsx` seeds its default from `oxygenPrimitives`, and if
 * that value lived beside the provider the two would import each other — a
 * cycle whose only symptom is `Cannot access 'oxygenPrimitives' before
...
function Button({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      data-ox-host-variant={type}
      data-ox-host-size={size}
      data-ox-host-danger={danger ? "" : undefined}
      className={cx("ox-host-btn", className)}
    >
      {children}
    </button>
  );
...
function Input({
  ...
  return (
    <span className={cx("ox-host-field", className)} data-ox-host-size={size}>
      {label ? (
        <label className="ox-host-field__label" htmlFor={id}>
          {label}
        </label>
      ) : null}
      <input
        id={id}
        type="text"
        className="ox-host-field__input"
        value={value}
        defaultValue={defaultValue}
        placeholder={placeholder}
        disabled={disabled}
        onChange={onChange}
        data-ox-host-status={status}
...
function Switch({
  ...
  return (
    <button
      ...
      disabled={disabled}
      data-ox-host-size={size}
      className={cx("ox-host-switch", className)}
      onClick={() => {
        if (!controlled) setOn(!on);
        onChange?.(!on);
      }}
    >
      <span className="ox-host-switch__thumb" />
    </button>
  );
}
...
function Checkbox({
  ...
  return (
    <label className={cx("ox-host-check", className)}>
      <input
        ref={ref}
        type="checkbox"
        checked={on}
...
function Select({
  ...
  return (
    <span className={cx("ox-host-field", className)} data-ox-host-size={size}>
      ...
      <select
        id={id}
        className="ox-host-field__input ox-host-select"
        value={value}
        defaultValue={defaultValue}
        disabled={disabled}
        aria-label={rest["aria-label"]}
...
function Tabs({ items, activeKey, defaultActiveKey, onChange, className, ...rest }: HostTabsProps) {
  ...
  return (
    <div className={cx("ox-host-tabs", className)}>
      <div className="ox-host-tabs__strip" role="tablist" aria-label={rest["aria-label"]}>
        {items.map((item) => {
...
export const oxygenPrimitives: HostPrimitives = {
  id: "oxygen",
  Button,
  Input,
  Switch,
